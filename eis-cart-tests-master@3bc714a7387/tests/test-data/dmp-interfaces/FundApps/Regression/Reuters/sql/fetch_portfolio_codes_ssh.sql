...
FROM   (
         SELECT MIN(acct_id) p1_id, 
                MAX(acct_id) p2_id
         FROM   (
                  select acct_id from ft_t_acst acst where stat_def_id = 'SSHFLAG' and end_tms is null
                  and exists (select 1 from ft_t_acid where acct_id_ctxt_typ = 'CRTSID' and acst.acct_id = acct_id and ACCT_ALT_ID = upper(ACCT_ALT_ID))
                ) 
       ) x,
       ft_t_acid p1,
       ft_t_acid p2
