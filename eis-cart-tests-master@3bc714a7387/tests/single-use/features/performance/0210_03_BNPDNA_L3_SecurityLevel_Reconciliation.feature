Feature: Performance L3 Attribution Security Level Historical Data Reconciliation: Legacy vs BNP DNA Platform
    ...
    @reconciliation @performance @performance_l3 @performance_l3_security_reconciliation
    Scenario Outline: Generate the Performance L3 highlighted reconciliation report - Case 1

        Then I generate the Performance L3 Security Level report with end date "<end-date>" and DNA Breakdown file "c:/tomwork/performance-l3/01-config/Request.csv"
        Then I generate the Performance L3 Security Level reconciliation reports at location "c:/tomwork/performance-l3/04-result/security" for result name "<result-name>" with report end date "<end-date>"
