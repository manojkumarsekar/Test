...
@dmp_wfoe @dmp_wfoe_transactions @dmp_wfoe_transactions_verify_data_transformation @tom_4205
Feature: 001 | China-WFOE | Transactions | Verify In-Memory Data Transformation

  Scenario: Verify Domain set defined for RCR Transactions Types are configured under fld_id = RCRTRNTP

    Given I expect value of column "IDMV_EDMV_COUNT" in the below SQL query equals to "4":
    """
    SELECT COUNT(*) AS IDMV_EDMV_COUNT FROM FT_T_IDMV I, FT_T_EDMV E
    WHERE I.INTRNL_DMN_VAL_ID = E.INTRNL_DMN_VAL_ID
    AND I.FLD_ID = 'RCRTRNTP'
    AND I.INTRNL_DMN_VAL_TXT IN ('BUY','BUYCLOSE','SELL','SELLSHORT')
    AND E.EXT_DMN_VAL_TXT IN ('P','C','S','SS')
    AND E.END_TMS IS NULL
    """

  Scenario: Verify In-Memory Data Transformation of CITICs Transactions file into RCR Transaction File

    Given I assign "001_esi_sc_transactions_verify_data.csv" to variable "INPUT_FILENAME"
  ...
  Scenario: Verify FOOTER record is appended in input file
    ...
    WHERE JOB_ID = '${JOB_ID}' AND RECORD_SEQ_NUM = '6'
    AND MAIN_ENTITY_ID = 'FOOTER'
    """

  Scenario: Verify Filter Record if CLIENT_SEC_TYPE is blank

    Then I expect value of column "FILTER_019517" in the below SQL query equals to "1":
    """
    SELECT COUNT(*) AS FILTER_019517 FROM FT_T_TRID
