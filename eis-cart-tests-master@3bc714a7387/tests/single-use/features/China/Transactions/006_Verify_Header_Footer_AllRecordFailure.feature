...
# TOM-4339 : Only for Transaction file, if CITICS provides files with no data in file or there are no qualified securities and hence the data is filtered,
# Generate Blank file with only Header and Footer.
...
@dmp_wfoe @dmp_wfoe_transactions @dmp_wfoe_transactions_verify_hf_allrecfail @tom_4205 @tom_4339
Feature: 006 | China-WFOE | Transactions | Verify blank file is not generated in case all records in input file fails/filtered

  Scenario: Verify blank file is not generated in case all records in input file fails/filtered

    Given I assign "006_esi_sc_transactions_verify_hf_allrecfail.csv" to variable "INPUT_FILENAME"
    And I assign "WFOEEISLTRANSN" to variable "OUTPUT_FILENAME"
    And I assign "006_007_WFOEEISLTRANSN_TEMPLATE.csv" to variable "MASTER_FILENAME"
    ...
    And I rename file "${PUBLISHING_DIR}/${OUTPUT_FILENAME}_${VAR_SYSDATE}.csv" as "${PUBLISHING_DIR}/006_${OUTPUT_FILENAME}_${VAR_SYSDATE}.csv" in the named host "dmp.ssh.inbound"
