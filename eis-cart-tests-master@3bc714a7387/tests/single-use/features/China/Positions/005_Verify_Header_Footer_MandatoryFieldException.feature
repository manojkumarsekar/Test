...
@dmp_wfoe @dmp_wfoe_positions @dmp_wfoe_positions_verify_hf_manfldexp @tom_4206
Feature: 005 | China-WFOE | Positions | Verify Header and Footer in case First and Last record in Input file fails for Mandatory Field Exception

  Scenario: Verify Header and Footer in case First and Last record in Input file fails for Mandatory Field Exception

    Given I assign "005_esi_sc_positions_verify_hf_manfldexcp.csv" to variable "INPUT_FILENAME"
    And I assign "WFOEEISLPOSITN" to variable "OUTPUT_FILENAME"
    And I assign "003_004_005_WFOEEISLPOSITN_TEMPLATE.csv" to variable "MASTER_FILENAME"
    ...
    And I rename file "${PUBLISHING_DIR}/${OUTPUT_FILENAME}_${VAR_SYSDATE}.csv" as "${PUBLISHING_DIR}/005_${OUTPUT_FILENAME}_${VAR_SYSDATE}.csv" in the named host "dmp.ssh.inbound"
