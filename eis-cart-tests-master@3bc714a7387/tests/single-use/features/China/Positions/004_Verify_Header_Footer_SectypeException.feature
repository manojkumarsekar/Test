...
@dmp_wfoe @dmp_wfoe_positions @dmp_wfoe_positions_verify_hf_sectypexp @tom_4206
Feature: 004 | China-WFOE | Positions | Verify Header and Footer in case First and Last record in Input file fails for Sec Type Exception

  Scenario: Verify Header and Footer in case First and Last record in Input file fails for Sec Type Exception

    Given I assign "004_esi_sc_positions_verify_hf_sectypexcp.csv" to variable "INPUT_FILENAME"
    ...
    And I rename file "${PUBLISHING_DIR}/${OUTPUT_FILENAME}_${VAR_SYSDATE}.csv" as "${PUBLISHING_DIR}/004_${OUTPUT_FILENAME}_${VAR_SYSDATE}.csv" in the named host "dmp.ssh.inbound"
