...
@02_tom_4555_bnp_dmp_idc3
Feature: IDC-3 - Intra Day Cash - Transaction File - Collat/ Margin

  Below Scenarios are handled as part of this feature:
  1. Validate the successful processing (No-Exception) of BNP-to-DMP Intraday Cash File (Collat-Margin)
  ...
  Scenario: TC_01: Initializing variables and generating the IDC-3 Cash test file for verification
    ...
    And I assign "ESIINTRADAY_TRN_TEST_FILE_CM.out" to variable "INPUT_FILENAME_CM"
    And I assign "ESIINTRADAY_TRN_TEMPLATE_COL_MGN.out" to variable "INPUT_TEMPLATE_COL_MGN"
  ...
  Scenario: TC_02: Processing the IDC-3 Cash test file for verification
    When I create input file "${INPUT_FILENAME_CM}" using template "${INPUT_TEMPLATE_COL_MGN}" with below codes from location "${testdata.path}"
    ...
    And I process files with below parameters and wait for the job to be completed
      | BUSINESS_FEED |                                      |
      | FILE_PATTERN  | ${INPUT_FILENAME_CM}                 |
      | MESSAGE_TYPE  | EIS_MT_BNP_INTRADAY_CASH_TRANSACTION |

    And I extract new job id from jblg table into a variable "JOB_ID"

  Scenario: TC_03: Verify no exceptions should be thrown post processing of BNP-to-DMP Intraday Cash File for Collat-Margin & total records are loaded from file
    ...
    Then I expect value of column "EXCEPTION_MSG_CHECK" in the below SQL query equals to "0":
      ...
      """
    Then I expect value of column "PROCESSED_RECORDS" in the below SQL query equals to "2":
      """
  ...
  Scenario: Extracting existing data for the first record in the input file
    ...
    Given I extract below values for row 2 from PSV file "${INPUT_FILENAME_CM}" in local folder "${testdata.path}/testdata" and assign to variables:
    ...
    And I extract below values for row 3 from PSV file "${INPUT_FILENAME_CM}" in local folder "${testdata.path}/testdata" and assign to variables:
      | CANCEL_IND | VAR_CANCEL_IND_CANC |

  Scenario Outline: TC_04: Intraday Cash File (Collat-Margin) Validations for input field: <ValidationStatus> :
    ...
    Examples: Expecting 'Pass' for each field from New Cash File vs Database
      ...
      | EXEC_TRN_CAT_SUB_TYP_CHECK   | SELECT CASE WHEN COUNT(DISTINCT(R.EXEC_TRN_CAT_SUB_TYP)) =1 THEN 'PASS' ELSE 'FAIL' END AS EXEC_TRN_CAT_SUB_TYP_CHECK FROM FT_T_EXTR R JOIN FT_T_ISID S ON R.INSTR_ID = S.INSTR_ID WHERE S.ISS_ID='${INSTR_ID1}' AND S.ID_CTXT_TYP='BNPLSTID' AND R.EXEC_TRN_CAT_SUB_TYP LIKE 'MMWD'                                                                                                 |
      | EXEC_TRN_CAT_TYP_CHECK       | SELECT CASE WHEN COUNT(DISTINCT(R.EXEC_TRN_CAT_TYP)) =1 THEN 'PASS' ELSE 'FAIL' END AS EXEC_TRN_CAT_TYP_CHECK FROM FT_T_EXTR R JOIN FT_T_ISID S ON R.INSTR_ID = S.INSTR_ID WHERE S.ISS_ID='${INSTR_ID1}' AND S.ID_CTXT_TYP='BNPLSTID' AND R.EXEC_TRN_CAT_TYP LIKE 'COLLAT/MARGIN'                                                                                                    |
