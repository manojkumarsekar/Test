...
@dmp_fundapps_functional @sec_regression @sec_regression_TC10 @fa_inbound @dmp_fundapps_regression
Feature: SCN10:Security Regression:  Load same listed security from RCR and BRS when Expiry date and Contract size not present in BRS file but is present in RCR file

  1) Load the File10 file without Expiry date and contract size fields and verify security created
  2) Load the RCR with Expiry date and Contract size and verify same security updated with Expiry date and Contract size details
  3) Load the same file 10 again and verify security not updated
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    ...
    And I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/TC10" with reference to "SECURITY_ID" column and assign to variables:
    ...
    And  I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${ISIN_ID}','${CUSIP_ID}','${SEDOL_CODE}'"

  Scenario: TC_2: Security update with RDM feed: Expiry date and Contract size are not present in BRS file

    Given I assign "esi_ADX_EOD_NON-ASIA_20190315_203007.sm.20190315 - 2.xml" to variable "INPUT_FILENAME_BRS"
    And I extract value from the xml file "${testdata.path}/TC10/${INPUT_FILENAME_BRS}" with tagName "CURRENCY" to variable "BRS_CURRENCY"
    And I extract value from the xml file "${testdata.path}/TC10/${INPUT_FILENAME_BRS}" with xpath "//CUSIP_ALIAS_set//CODE[text()='70']/../IDENTIFIER" to variable "BRS_ISIN_ID"
    And I extract value from the xml file "${testdata.path}/TC10/${INPUT_FILENAME_BRS}" with xpath "//CUSIP2_set//CODE[text()='C']/../IDENTIFIER" to variable "BRS_SEDOL"
    And I extract value from the xml file "${testdata.path}/TC10/${INPUT_FILENAME_BRS}" with tagName "DESC_INSTMT" to variable "BRS_DESCRIPTION"
    And I extract value from the xml file "${testdata.path}/TC10/${INPUT_FILENAME_BRS}" with tagName "SM_SEC_TYPE" to variable "BRS_SM_SEC_TYPE"
    And I assign "EIS_BRS_DMP_SECURITY" to variable "BRS_LAST_CHG_USR"
    When I copy files below from local folder "${testdata.path}/TC10" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | ${INPUT_FILENAME_BRS} |
