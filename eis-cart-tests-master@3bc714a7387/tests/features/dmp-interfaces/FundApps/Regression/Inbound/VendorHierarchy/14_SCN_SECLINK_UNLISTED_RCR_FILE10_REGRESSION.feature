...
@dmp_fundapps_functional @sec_regression @sec_regression_TC14 @fa_inbound @dmp_fundapps_regression
Feature: SCN014 : Security Regression: Load same unlisted security from BRS and RCR , But Expiry date and contract size are different  in RCR file

  1) Load the Aladdin file 10 and verify security created
  2) Load the same RCR file with different Expiry date and contract size and separate security created
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    ...
    And I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/TC14" with reference to "SECURITY_ID" column and assign to variables:
    ...
    And I extract value from the xml file "${testdata.path}/TC14/${INPUT_FILENAME}" with tagName "CUSIP" to variable "BCUSIP"
  ...
  Scenario: TC_2: security should created with BRS feed
    ...
    And I extract value from the xml file "${testdata.path}/TC14/${INPUT_FILENAME_BRS}" with tagName "CURRENCY" to variable "BRS_CURRENCY"
    And I extract value from the xml file "${testdata.path}/TC14/${INPUT_FILENAME_BRS}" with xpath "//CUSIP_ALIAS_set//CODE[text()='70']/../IDENTIFIER" to variable "BRS_ISIN_ID"
    And I extract value from the xml file "${testdata.path}/TC14/${INPUT_FILENAME_BRS}" with xpath "//CUSIP2_set//CODE[text()='C']/../IDENTIFIER" to variable "BRS_SEDOL"
    And I extract value from the xml file "${testdata.path}/TC14/${INPUT_FILENAME_BRS}" with tagName "CONTRACT_SIZE" to variable "BRS_CONTRACT_SIZE"
    And I extract value from the xml file "${testdata.path}/TC14/${INPUT_FILENAME_BRS}" with tagName "DESC_INSTMT" to variable "BRS_DESCRIPTION"
    And I extract value from the xml file "${testdata.path}/TC14/${INPUT_FILENAME_BRS}" with tagName "MATURITY" to variable "BRS_MAT_EXP_TMS"
    And I extract value from the xml file "${testdata.path}/TC14/${INPUT_FILENAME_BRS}" with tagName "SM_SEC_TYPE" to variable "BRS_SM_SEC_TYPE"
    And I assign "EIS_BRS_DMP_SECURITY" to variable "BRS_LAST_CHG_USR"
    When I copy files below from local folder "${testdata.path}/TC14" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | ${INPUT_FILENAME_BRS} |
