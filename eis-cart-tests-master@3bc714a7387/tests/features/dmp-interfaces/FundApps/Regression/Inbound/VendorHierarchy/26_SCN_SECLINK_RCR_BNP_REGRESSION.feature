...
@dmp_fundapps_functional @sec_regression @sec_regression_TC26 @fa_inbound @dmp_fundapps_regression
Feature: SCN026 : Security Regression: Load the same listed security from BNP and RCR file without ISIN,CUSIP,Maturity date

  1) Load the RCR file without ISIN,CUSIP,Maturity date and verify security created
  2) Load the BNP Security File with  ISIN , CUSIP,Maturity date and verify same security updated with BNP details
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    ...
    And I assign "BOCIEISLINSTMT20190513_26.csv" to variable "INPUT_FILENAME"
    And I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/TC26" with reference to "SECURITY_ID" column and assign to variables:
    ...
    And I assign "ESISODP_SEC_1_20190510_26.out" to variable "BNP_INPUT_FILENAME"
    And I extract below values for row 2 from PSV file "${BNP_INPUT_FILENAME}" in local folder "${testdata.path}/TC26" and assign to variables:
    ...
    And I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${ISS_ID}','${SEDOL_CODE}','${BNP_INSTR_ID}','${BNP_ISIN}','${BNP_SEDOL}','${BNP_HIP_SECURITY_CODE}'"

  Scenario: TC_2:New Security Creation with RCR feed:New Security should be created when Load listed security from RCR

    Given I assign "BOCIEISLINSTMT20190513_26.csv" to variable "INPUT_FILENAME"
    When I copy files below from local folder "${testdata.path}/TC26" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | ${INPUT_FILENAME} |
  ...
  Scenario: Clear the data after tests
    Then I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${ISS_ID}','${SEDOL_CODE}','${BNP_INSTR_ID}','${BNP_ISIN}','${BNP_SEDOL}','${BNP_HIP_SECURITY_CODE}'"
