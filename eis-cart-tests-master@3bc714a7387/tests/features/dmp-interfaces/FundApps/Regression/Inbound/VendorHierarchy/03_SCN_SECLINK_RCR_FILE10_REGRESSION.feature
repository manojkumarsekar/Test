...
@dmp_fundapps_functional @sec_regression @sec_regression_TC03 @fa_inbound @dmp_fundapps_regression
Feature: SCN03:Security Regression: Load same listed security from RCR and BRS when ISIN is not present in BRS file but is present in RCR file

  1) Load the Aladdin file 10 without ISIN and verify Security created
  2) Load the same RCR file again and verify same Security Updated with ISIN
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    ...
    And I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/TC3" with reference to "SECURITY_ID" column and assign to variables:
    ...
    And I extract value from the xml file "${testdata.path}/TC3/${INPUT_FILENAME_BRS}" with tagName "CUSIP" to variable "BCUSIP"
    And I extract value from the xml file "${testdata.path}/TC3/${INPUT_FILENAME_BRS}" with xpath "//CUSIP_ALIAS_set//CODE[text()='70']/../IDENTIFIER" to variable "BRS_ISIN_ID"
    And I extract value from the xml file "${testdata.path}/TC3/${INPUT_FILENAME_BRS}" with xpath "//CUSIP2_set//CODE[text()='C']/../IDENTIFIER" to variable "BRS_SEDOL"
    And I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${SEDOL_CODE}','${CUSIP_ID}','${ISIN_ID}','${BRS_SEDOL}','${BCUSIP}'"

  Scenario: TC_2: Security update with BRS feed: ISIN is not present in BRS file

    Given I extract value from the xml file "${testdata.path}/TC3/${INPUT_FILENAME_BRS}" with tagName "CURRENCY" to variable "BRS_CURRENCY"
    And I extract value from the xml file "${testdata.path}/TC3/${INPUT_FILENAME_BRS}" with tagName "CONTRACT_SIZE" to variable "BRS_CONTRACT_SIZE"
    And I extract value from the xml file "${testdata.path}/TC3/${INPUT_FILENAME_BRS}" with tagName "DESC_INSTMT" to variable "BRS_DESCRIPTION"
    And I extract value from the xml file "${testdata.path}/TC3/${INPUT_FILENAME_BRS}" with tagName "MATURITY" to variable "BRS_MAT_EXP_TMS"
    And I extract value from the xml file "${testdata.path}/TC3/${INPUT_FILENAME_BRS}" with tagName "SM_SEC_TYPE" to variable "BRS_SM_SEC_TYPE"
    And I assign "EIS_BRS_DMP_SECURITY" to variable "BRS_LAST_CHG_USR"
    When I copy files below from local folder "${testdata.path}/TC3" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | ${INPUT_FILENAME_BRS} |
  ...
  Scenario: TC_3:New ISIN Creation with RCR feed:ISIN and CUSIP should get created when Load listed security from RCR
  ...
  Scenario: TC_4: Security no update with BRS feed: Security should not be updated when load the same RDM file again
    ...
    And I expect value of column "BRS_CONTRACT_COUNT" in the below SQL query equals to "1":
      """
        SELECT COUNT(*) AS BRS_CONTRACT_COUNT FROM FT_T_ISSU
        WHERE INSTR_ID=(
                        SELECT INSTR_ID FROM FT_T_ISID WHERE ISS_ID = '${BCUSIP}'AND ID_CTXT_TYP='BCUSIP' AND END_TMS IS NULL
                       )
        AND ISS_UT_CQTY='${BRS_CONTRACT_SIZE}'
        AND DENOM_CURR_CDE='${BRS_CURRENCY}'
        AND MAT_EXP_TMS=TO_DATE('${BRS_MAT_EXP_TMS}','MM/DD/YYYY')
  ...
  Scenario: Clear the data after tests
    Then I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${SEDOL_CODE}','${CUSIP_ID}','${ISIN_ID}','${BRS_SEDOL}','${BCUSIP}'"
