...
@dmp_fundapps_functional @sec_regression @sec_regression_TC07 @fa_inbound @dmp_fundapps_regression
Feature: SCN07:Security Regression :Load same listed security from RCR and BRS when CUSIP is not present in RCR file but present in BRS file

  1) Load the RCR file without CUSIP and verify Security created
  2) Load the Aladdin file 10 and verify same Security Updated with all BRS details
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    ...
    And I assign "BOCIEISLINSTMT20191218_07.csv" to variable "INPUT_FILENAME"
    And I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/TC7" with reference to "SECURITY_ID" column and assign to variables:
    ...
    And I assign "esi_ADX_EOD_NON-ASIA_20190315_203007.sm.20190315 -07.xml" to variable "INPUT_FILENAME_BRS"
    And I extract value from the xml file "${testdata.path}/TC7/${INPUT_FILENAME_BRS}" with xpath "//CUSIP_ALIAS_set//CODE[text()='70']/../IDENTIFIER" to variable "BRS_ISIN_ID"
    And I extract value from the xml file "${testdata.path}/TC7/${INPUT_FILENAME_BRS}" with xpath "//CUSIP2_set//CODE[text()='C']/../IDENTIFIER" to variable "BRS_SEDOL"
    And I extract value from the xml file "${testdata.path}/TC7/${INPUT_FILENAME_BRS}" with xpath "//CUSIP2_record//CODE[text()='A']/../IDENTIFIER" to variable "BRS_CUSIP"
    And I extract value from the xml file "${testdata.path}/TC7/${INPUT_FILENAME_BRS}" with tagName "CUSIP" to variable "BCUSIP"
    And I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${ISIN_ID}','${BCUSIP}','${SEDOL_CODE}','${BRS_ISIN_ID}'"

  Scenario: TC_2:New Security Creation with RCR feed:New Security should be created when Load listed security from RCR CUSIP is not present

    Given I assign "BOCIEISLINSTMT20191218_07.csv" to variable "INPUT_FILENAME"
    When I copy files below from local folder "${testdata.path}/TC7" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | ${INPUT_FILENAME} |
    ...
    And I expect value of column "SEDOL_COUNT" in the below SQL query equals to "1":
      """
       ...
       AND TRUNC(LAST_CHG_TMS)=TRUNC(SYSDATE)
      """
    And I expect value of column "CUSIP_COUNT" in the below SQL query equals to "0":
      """
  ...
  Scenario: TC_3: Security update with BRS file- Security should get updated with parameters from BRS File
    ...
    Given I extract value from the xml file "${testdata.path}/TC7/${INPUT_FILENAME_BRS}" with tagName "CURRENCY" to variable "BRS_CURRENCY"
    And I extract value from the xml file "${testdata.path}/TC7/${INPUT_FILENAME_BRS}" with tagName "CONTRACT_SIZE" to variable "BRS_CONTRACT_SIZE"
    And I extract value from the xml file "${testdata.path}/TC7/${INPUT_FILENAME_BRS}" with tagName "DESC_INSTMT" to variable "BRS_DESCRIPTION"
    And I extract value from the xml file "${testdata.path}/TC7/${INPUT_FILENAME_BRS}" with tagName "MATURITY" to variable "BRS_MAT_EXP_TMS"
    And I extract value from the xml file "${testdata.path}/TC7/${INPUT_FILENAME_BRS}" with tagName "SM_SEC_TYPE" to variable "BRS_SM_SEC_TYPE"
  ...
  Scenario: Clear the data after tests
    Then I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${ISIN_ID}','${BCUSIP}','${SEDOL_CODE}','${BRS_ISIN_ID}'"