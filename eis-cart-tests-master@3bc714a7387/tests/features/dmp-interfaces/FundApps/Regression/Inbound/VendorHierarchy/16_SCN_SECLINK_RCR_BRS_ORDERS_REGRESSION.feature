...
@dmp_fundapps_functional @sec_regression @sec_regression_TC16 @fa_inbound @dmp_fundapps_regression
Feature: SCN16 :Security Regression: Load the same listed security from RCR and BRS Orders and All fields match in both files

  1) Load the RCR file and verify security created
  2) Load the Aladdin Orders filend verify same security updated with order file details
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    ...
    And I assign "BOCIEISLINSTMT20191005.csv" to variable "INPUT_FILENAME"
    And I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/TC16" with reference to "SECURITY_ID" column and assign to variables:
    ...
    And I assign "esi_orders.20190510_0835.xml" to variable "INPUT_FILENAME"
    And I extract value from the xml file "${testdata.path}/TC16/${INPUT_FILENAME}" with tagName "CUSIP" to variable "BCUSIP"
  ...
  Scenario: TC_2:New Security Creation with RCR feed:New Security should be created when Load listed security from RCR
    ...
    Given I assign "BOCIEISLINSTMT20191005.csv" to variable "INPUT_FILENAME"
    When I copy files below from local folder "${testdata.path}/TC16" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | ${INPUT_FILENAME} |
  ...
  Scenario: TC_3: Security update with BRS feed: Security should be updated when same listed security from BRS Orders

    Given I assign "esi_orders.20190510_0835.xml" to variable "INPUT_FILENAME_BRS"
    And I extract value from the xml file "${testdata.path}/TC16/${INPUT_FILENAME_BRS}" with tagName "CUSIP" to variable "BCUSIP"
    And I extract value from the xml file "${testdata.path}/TC16/${INPUT_FILENAME_BRS}" with tagName "CURRENCY" to variable "BRS_CURRENCY"
    And I extract value from the xml file "${testdata.path}/TC16/${INPUT_FILENAME_BRS}" with tagName "ISIN" to variable "BRS_ISIN_ID"
    And I extract value from the xml file "${testdata.path}/TC16/${INPUT_FILENAME_BRS}" with tagName "SEDOL" to variable "BRS_SEDOL"
    And I extract value from the xml file "${testdata.path}/TC16/${INPUT_FILENAME_BRS}" with tagName "SEC_DESC1" to variable "BRS_DESCRIPTION"
    And I extract value from the xml file "${testdata.path}/TC16/${INPUT_FILENAME_BRS}" with tagName "MATURITY" to variable "BRS_MAT_EXP_TMS"
    And I extract value from the xml file "${testdata.path}/TC16/${INPUT_FILENAME_BRS}" with tagName "SM_SEC_TYPE" to variable "BRS_SM_SEC_TYPE"
