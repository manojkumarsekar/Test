...
@dmp_fundapps_functional @sec_regression @sec_regression_TC12 @fa_inbound @dmp_fundapps_regression
Feature: SCN012:Security Regression: Load same unlisted security from RCR and BRS, But Expiry date and contract size not present in RCR file

  1) Load the Unlisted RCR file with Expiry date and contract size not present and verify security created
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    ...
    And I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/TC12" with reference to "SECURITY_ID" column and assign to variables:
    ...
    And I extract value from the xml file "${testdata.path}/TC12/${INPUT_FILENAME}" with tagName "CUSIP" to variable "BCUSIP"
    And  I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${SECURITY_ID}','${BCUSIP}'"

  Scenario: TC_2:New Security Creation with unlisted RCR feed:New Security should be created when Load unlisted security from RCR when Expiry date and contract size not present in RCR file

    Given I assign "BOCIEISLINSTMT20181218.csv" to variable "INPUT_FILENAME"
    When I copy files below from local folder "${testdata.path}/TC12" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | ${INPUT_FILENAME} |
  ...
  Expiry date and Contract size are present in BRS unlisted security file

    Given I assign "esi_ADX_EOD_NON-ASIA_20190315_203007.sm.20190315 - 2.xml" to variable "INPUT_FILENAME_BRS"
    And I extract value from the xml file "${testdata.path}/TC12/${INPUT_FILENAME_BRS}" with tagName "CURRENCY" to variable "BRS_CURRENCY"
    And I extract value from the xml file "${testdata.path}/TC12/${INPUT_FILENAME_BRS}" with xpath "//CUSIP_ALIAS_set//CODE[text()='70']/../IDENTIFIER" to variable "BRS_ISIN_ID"
    And I extract value from the xml file "${testdata.path}/TC12/${INPUT_FILENAME_BRS}" with xpath "//CUSIP2_set//CODE[text()='C']/../IDENTIFIER" to variable "BRS_SEDOL"
    And I extract value from the xml file "${testdata.path}/TC12/${INPUT_FILENAME_BRS}" with tagName "CONTRACT_SIZE" to variable "BRS_CONTRACT_SIZE"
    And I extract value from the xml file "${testdata.path}/TC12/${INPUT_FILENAME_BRS}" with tagName "DESC_INSTMT" to variable "BRS_DESCRIPTION"
    And I extract value from the xml file "${testdata.path}/TC12/${INPUT_FILENAME_BRS}" with tagName "MATURITY" to variable "BRS_MAT_EXP_TMS"
    And I extract value from the xml file "${testdata.path}/TC12/${INPUT_FILENAME_BRS}" with tagName "SM_SEC_TYPE" to variable "BRS_SM_SEC_TYPE"
