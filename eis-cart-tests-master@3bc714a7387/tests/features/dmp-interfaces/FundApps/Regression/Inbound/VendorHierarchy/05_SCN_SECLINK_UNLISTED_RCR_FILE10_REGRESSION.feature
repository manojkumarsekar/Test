...
@dmp_fundapps_functional @sec_regression @sec_regression_TC05 @fa_inbound @dmp_fundapps_regression
Feature: SCN05:Security Regression: Load same unlisted security from RCR and BRS

  1) Load the Unlisted RCR file and verify Security created
  2) Load the Unlisted Aladdin file 10 and verify separate Security created
  3) Load the same Unlisted RCR file again and verify security not updated
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    ...
    And I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/TC5" with reference to "SECURITY_ID" column and assign to variables:
    ...
    And I extract value from the xml file "${testdata.path}/TC5/${INPUT_FILENAME}" with tagName "CUSIP" to variable "BCUSIP"
  ...
  Scenario: TC_2:New Security Creation with unlisted RCR feed:New Security should be created when Load unlisted security from RCR
    ...
    When I copy files below from local folder "${testdata.path}/TC5" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | ${INPUT_FILENAME} |
  ...
  Scenario: TC_3: Separate security should created with BRS feed when same unlisted BRS file loaded

    Given I assign "esi_ADX_EOD_NON-ASIA_20190315_203007.sm.20190315 - 2.xml" to variable "INPUT_FILENAME_BRS"
    And I extract value from the xml file "${testdata.path}/TC5/${INPUT_FILENAME_BRS}" with tagName "CURRENCY" to variable "BRS_CURRENCY"
    And I extract value from the xml file "${testdata.path}/TC5/${INPUT_FILENAME_BRS}" with xpath "//CUSIP_ALIAS_set//CODE[text()='70']/../IDENTIFIER" to variable "BRS_ISIN_ID"
    And I extract value from the xml file "${testdata.path}/TC5/${INPUT_FILENAME_BRS}" with xpath "//CUSIP2_set//CODE[text()='C']/../IDENTIFIER" to variable "BRS_SEDOL"
    And I extract value from the xml file "${testdata.path}/TC5/${INPUT_FILENAME_BRS}" with tagName "CONTRACT_SIZE" to variable "BRS_CONTRACT_SIZE"
    And I extract value from the xml file "${testdata.path}/TC5/${INPUT_FILENAME_BRS}" with tagName "DESC_INSTMT" to variable "BRS_DESCRIPTION"
    And I extract value from the xml file "${testdata.path}/TC5/${INPUT_FILENAME_BRS}" with tagName "MATURITY" to variable "BRS_MAT_EXP_TMS"
    And I extract value from the xml file "${testdata.path}/TC5/${INPUT_FILENAME_BRS}" with tagName "SM_SEC_TYPE" to variable "BRS_SM_SEC_TYPE"
