...
@dmp_rcrlbu_THANA_funds @eisdev_5218 @dmp_fundapps_functional @fund_apps_funds @eisdev_5511
Feature: EISDEV-5218: Funds RCRLBU THANA file load (Golden Source)
  ...
  Scenario: TC_1: Clear ACID data and assign variables
    ...
    And I assign "THANEISLFUNDLE20170331_test.csv" to variable "INPUT_FILENAME"
    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/THANA" with reference to "Fund Id" column and assign to variables:
      | Fund Id                                        | ACT_ID    |
