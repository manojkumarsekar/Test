...
@tom_4461 @dmp_rcrlbu_positions_tmbam @dmp_fundapps_functional @fa_inbound_positions @fa_inbound @dmp_fundapps_regression @eisdev_6913
Feature: Positions RCRLBU TMBAM file load (Golden Source)
  Positions creation in DMP through feed file load from TMBAM RCRLBU.

  #Prerequisites
  Scenario: TC_1: Clear data for RCRLBU Position for Data Source TMBAM and initial variable assignment
    ...
    And  I assign "TBAMEISLPOSITN_${VAR_SYSDATE}.csv" to variable "INPUT_FILENAME"
    ...
    And I create input file "${INPUT_FILENAME}" using template "TBAMEISLPOSITN_Template.csv" from location "${testdata.path}"
  ...
  Scenario: TC_2: Load pre-requisite Fund Data before file
    ...
    When I copy files below from local folder "${testdata.path}/testdata/TMBAM" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | TBAMEISLFUNDLE20190322.csv |
...
#Verification of successful File load
  ...
  Scenario: TC_4: File load for RCRLBU Position for Data Source TMBAM
