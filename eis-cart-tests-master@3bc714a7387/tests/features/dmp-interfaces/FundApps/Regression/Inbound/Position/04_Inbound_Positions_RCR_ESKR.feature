...
@tom_4461 @dmp_rcrlbu_positions_eskr @dmp_fundapps_functional @fa_inbound_positions @fa_inbound @dmp_fundapps_regression
Feature: Positions RCRLBU EastSpring Korea file load (Golden Source)
  Positions creation in DMP through feed file load from Eastspring Korea RCRLBU.

  #Prerequisites
  Scenario: TC_1: Clear data for RCRLBU Position for Data Source ESKR and initial variable assignment
    ...
    And  I assign "EIMKEISLPOSITN_${VAR_SYSDATE}.csv" to variable "INPUT_FILENAME"
    ...
    And I create input file "${INPUT_FILENAME}" using template "EIMKEISLPOSITN_Template.csv" from location "${testdata.path}"
    ...
    And I assign "EIMKORCDE" to variable "ISSU_ID_CTXT"
    And I assign "KOREAEOD" to variable "RQSTR_ID"
  ...
  Scenario: TC_2: Load pre-requisite Fund Data before file
    When I copy files below from local folder "${testdata.path}/testdata/ESKR" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | EIMKEISLFUNDLE20190322.csv |
  ...
  Scenario: TC_4: File load for RCRLBU Position for Data Source Eastspring Korea
