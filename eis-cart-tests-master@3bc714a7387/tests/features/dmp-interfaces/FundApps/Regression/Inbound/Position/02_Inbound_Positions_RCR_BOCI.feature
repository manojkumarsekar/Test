...
# Regression was failing due to missing check of ISS_ID in validation, this could come due to order in which feature gets executed


@gc_interface_securities @gc_interface_positions @gc_interface_funds
@dmp_regression_integrationtest
@dmp_fundapps_regression
@tom_4461 @dmp_rcrlbu_positions_boci @dmp_fundapps_functional @fa_inbound_positions @fa_inbound @eisdev_5396
Feature: Positions RCRLBU BOCI file load (Golden Source)

  Positions creation in DMP through feed file load from BOCI RCRLBU.

  #Prerequisites
  Scenario: TC_1: Clear data for RCRLBU Position for Data Source BOCI and initial variable assignment
    ...
    And  I assign "BOCIEISLPOSITN_${VAR_SYSDATE}.csv" to variable "INPUT_FILENAME"
    ...
    And I create input file "${INPUT_FILENAME}" using template "BOCIEISLPOSITN_Template.csv" from location "${testdata.path}"
    ...
    And I assign "BOCICODE" to variable "ISSU_ID_CTXT"
    And I assign "BOCIEOD" to variable "RQSTR_ID"
  ...
  Scenario: TC_4: File load for RCRLBU Position for Data Source BOCI
  ...
  Scenario: TC_6: Verification of BALH table for the number of positions loaded to dmp from position file
