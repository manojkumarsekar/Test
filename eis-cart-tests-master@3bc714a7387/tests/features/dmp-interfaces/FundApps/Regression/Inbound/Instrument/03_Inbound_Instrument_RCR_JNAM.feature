...
@tom_4265 @03_inbound_rcr_jnam @dmp_fundapps_functional @fund_apps_instrument @fa_inbound
Feature: To verify that DMP receive the inbound instrument file data from the entity JNAM
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    Given I assign "JNAMEISLINSTMT20190325.csv" to variable "INPUT_FILENAME"
    And I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/testdata" with reference to "SECURITY_ID" column and assign to variables:
      ...
      | CUSIP                         | CUSIP_CODE              |
      | SOURCE                        | SOURCE_CODE             |
      | VOTING_RIGHTS_INDICATOR       | VOTING_RIGHTS_TYP_VALUE |
      | NEXT_CONVERSION_DATE          | NXT_CNVR_PRD_START_DTE  |
      | U/L_SEC_ID                    | UL_SEC_ID               |
      | EXPIRY_DATE                   | MAT_EXP_TMS             |
      | CV_TERMS_NUMERATOR            | CNVR_RATIO_NUMER_NUM    |
      | CV_TERMS_DENOMINATOR          | CNVR_RATIO_DENOM_NUM    |
      | CONTRACT_SIZE                 | ISS_UT_CQTY             |
      | OPTION_TYPE                   | CALL_PUT_TYP            |
      | CURRENCY_OF_DENOMINATION      | DENOM_CURR_CODE         |
      | SECURITY_TYPE/INSTRUMENT_TYPE | SEC_TYP                 |

    Then I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${ISS_ID}','${SEDOL_CODE}','${CUSIP_CODE}','${ISIN}'"

  Scenario: TC_2: Load Security file for JNAM
  ...
  Scenario: TC_3: Data Verifications for JNAM
  To verify  the inbound instrument file load in to DMP  from the entity JNAM
    ...
    AND JOB_MSG_TYP='EIS_MT_JNAM_DMP_SECURITY' AND JOB_ID = '${JOB_ID}' AND JOB_INPUT_TXT LIKE '%${INPUT_FILENAME}'
    """

  Scenario: TC_4: Data Verifications for JNAM
  To verify the Security_id from file in to DMP
    ...
    WHERE ID_CTXT_TYP = 'PPMJNAMCDE' AND ISS_ID='${ISS_ID}'
    AND TRUNC(LAST_CHG_TMS)=TRUNC(SYSDATE)
    AND END_TMS IS NULL
    """

  Scenario: TC_5: Data Verifications for JNAM : To verify DESC_USAGE_TYP (Description) in FT_T_ISDE table.
  ...
  Scenario: TC_6: Data Verifications for JNAM: To verify the ISIN 
  ...
  Scenario: TC_7: Data Verifications for JNAM :To verify the SEDOL
  ...
  Scenario: TC_8: Data Verifications for JNAM:To verify the CUSIP
  ...
  Scenario: TC_9: Data Verifications for JNAM:To verify the  Source BU Code
  ...
  Scenario: TC_10: Data Verifications for JNAM:To verify the  Voting Rights Indicator
  ...
  Scenario: TC_11: Data Verifications for JNAM:To verify the  Next Conversion Date
  ...
  Scenario: TC_12: Data Verifications for JNAM:To verify the U/L sec ID

    Then I expect value of column "ISS_PART_RL_TYP_COUNT" in the below SQL query equals to "0":
    """
      SELECT COUNT(*) AS ISS_PART_RL_TYP_COUNT FROM FT_T_RISS
      WHERE INSTR_ID=(
                      SELECT INSTR_ID FROM FT_T_ISID WHERE ID_CTXT_TYP = 'PPMJNAMCDE' AND ISS_ID='${ISS_ID}' AND END_TMS IS NULL
                      )
  ...
  Scenario: TC_13: Data Verifications for JNAM:To verify the Expiry Date,Contract Size,Currency of Denomination and Security Type/Instrument Type  and
  ...
  Scenario: TC_14: Data Verifications for JNAM:To verify the CV Terms Numerator and CV Terms Denominator

    Then I expect value of column "CVTERMS_NUM_COUNT" in the below SQL query equals to "0":
    """
      SELECT COUNT(*) AS CVTERMS_NUM_COUNT FROM FT_T_RIST
      WHERE INSTR_ID=(
                      SELECT INSTR_ID FROM FT_T_ISID WHERE ID_CTXT_TYP = 'PPMJNAMCDE' and ISS_ID='${ISS_ID}' AND END_TMS IS NULL
                      )
  ...
  Scenario: TC_15: Data Verifications for JNAM:To verify the Option type

    Then I expect value of column "OPT_COUNT" in the below SQL query equals to "0":
    """
      SELECT COUNT(*) AS OPT_COUNT FROM FT_T_OPCH
      WHERE INSTR_ID=(
                      SELECT INSTR_ID FROM FT_T_ISID WHERE ID_CTXT_TYP = 'PPMJNAMCDE' and ISS_ID='${ISS_ID}' AND END_TMS IS NULL
                      )
  ...
  Scenario: TC_16: Data Verifications for M&G:To verify the Security Type/Instrument Type
    ...
    """
      SELECT COUNT(*) AS SECURITY_TYPE_COUNT FROM FT_T_INCL
      WHERE INDUS_CL_SET_ID = 'JNAMSCTYP' AND CL_VALUE = '${SEC_TYP}' AND END_TMS IS NULL
    """
