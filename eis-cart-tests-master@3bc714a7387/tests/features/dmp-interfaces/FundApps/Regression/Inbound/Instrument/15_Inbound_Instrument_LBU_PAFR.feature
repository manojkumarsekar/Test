...
@tom_5217 @fa_inbound @09_inbound_rcr_pafr @dmp_fundapps_functional @fund_apps_instrument
Feature: To verify that DMP receive the inbound instrument file data from the entity PAFR
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    Given I assign "PAFREISLINSTMT20191031.csv" to variable "INPUT_FILENAME"
  ...
  Scenario: TC_2: Load MNG file PAFREISLINSTMT20191031.csv
  ...
  Scenario: TC_3: Data Verifications for 'PAFR' :To verify  the inbound instrument file load in to DMP  from the entity 'PAFR'
    Then I expect value of column "JOB_ID" in the below SQL query equals to "1":
      ...
      AND JOB_MSG_TYP='EIS_MT_PAFR_DMP_SECURITY' AND JOB_ID = '${JOB_ID}' AND JOB_INPUT_TXT LIKE '%${INPUT_FILENAME}'
      """

  Scenario: TC_4: Data Verifications for 'PAFR: To verify the Security_id from file in to DMP

    Then I expect value of column "ISS_ID" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS ISS_ID FROM FT_T_ISID
      WHERE ID_CTXT_TYP = 'PAFRCODE'
      AND ISS_ID='${ISS_ID}'
      AND TRUNC(LAST_CHG_TMS)=TRUNC(SYSDATE)
      AND END_TMS IS NULL
      """

  Scenario: TC_5: Data Verifications for 'PAFR : To verify DESC_USAGE_TYP (Description) in FT_T_ISDE table.
    Then I expect value of column "DESC_USAGE_TYP_COUNT" in the below SQL query equals to "1":
      """
       ...
       AND INSTR_ID = (SELECT INSTR_ID FROM FT_T_ISID WHERE ID_CTXT_TYP = 'PAFRCODE' AND ISS_ID='${ISS_ID}' AND END_TMS IS NULL)
       AND TRUNC(LAST_CHG_TMS)=TRUNC(SYSDATE)
       AND END_TMS IS NULL
      """

  Scenario: TC_6: Data Verifications for 'PAFR: To verify the ISIN 
  ...
  Scenario: TC_7: Data Verifications for 'PAFR :To verify the SEDOL
  ...
  Scenario: TC_9: Data Verifications for PAFR:To verify the  Source BU Code
  ...
  Scenario: TC_10: Data Verifications for PAFR :To verify the  Voting Rights Indicator
  ...
  Scenario: TC_11: Data Verifications for PAFR:To verify the  Next Conversion Date
    ...
    """
      ...
      WHERE INSTR_ID=(
                      SELECT INSTR_ID FROM FT_T_ISID WHERE ID_CTXT_TYP = 'PAFRCODE' and ISS_ID='${ISS_ID}' AND END_TMS IS NULL
                      )
      AND NXT_CNVR_PRD_START_DTE IS NULL
      AND TRUNC(LAST_CHG_TMS)=TRUNC(SYSDATE)
    """

  Scenario: TC_13: Data Verifications for PAFR:To verify the Expiry Date,Contract Size and Currency of Denomination
    ...
    """
      ...
      INSTR_ID=(SELECT INSTR_ID FROM FT_T_ISID WHERE ID_CTXT_TYP = 'PAFRCODE' AND ISS_ID='${ISS_ID}' AND END_TMS IS NULL)
      AND MAT_EXP_TMS IS NULL AND ISS_UT_CQTY IS NULL AND DENOM_CURR_CDE='${DENOM_CURR_CODE}'
    """

  Scenario: TC_14: Data Verifications for PAFR:To verify the CV Terms Numerator and CV Terms Denominator
  ...
  Scenario: TC_15: Data Verifications for PAFR:To verify the Option type
  ...
  Scenario: TC_16: Data Verifications for PAFR:To verify the Security Type/Instrument Type
    ...
    """
      ...
      INDUS_CL_SET_ID = 'PAFRSCTYPE' AND CL_VALUE = '${SEC_TYP}' AND END_TMS IS NULL
    """
