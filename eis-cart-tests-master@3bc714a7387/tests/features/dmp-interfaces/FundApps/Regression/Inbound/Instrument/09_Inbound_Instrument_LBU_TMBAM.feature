...
@fa_inbound @tom_4265 @09_inbound_rcr_tmbam @dmp_fundapps_functional @fund_apps_instrument
Feature: To verify that DMP receive the inbound instrument file data from the entity TMBAM
  ...
  Scenario: TC_1:Prerequisites before running actual tests
    Given I assign "TBAMEISLINSTMT20190325.csv" to variable "INPUT_FILENAME"
  ...
  Scenario: TC_2: Load MNG file TBAMEISLINSTMT20190325.csv
  ...
  Scenario: TC_3: Data Verifications for 'TMBAM' :To verify  the inbound instrument file load in to DMP  from the entity 'TMBAM'
    Then I expect value of column "JOB_ID" in the below SQL query equals to "1":
      ...
      AND JOB_MSG_TYP='EIS_MT_TMBAM_DMP_SECURITY' AND JOB_ID = '${JOB_ID}' AND JOB_INPUT_TXT LIKE '%${INPUT_FILENAME}'
      """

  Scenario: TC_4: Data Verifications for 'TMBAM: To verify the Security_id from file in to DMP

    Then I expect value of column "ISS_ID" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS ISS_ID FROM FT_T_ISID
      WHERE ID_CTXT_TYP = 'TMBAMCDE'
      AND ISS_ID='${ISS_ID}'
      AND TRUNC(LAST_CHG_TMS)=TRUNC(SYSDATE)
      AND END_TMS IS NULL
      """

  Scenario: TC_5: Data Verifications for 'TMBAM : To verify DESC_USAGE_TYP (Description) in FT_T_ISDE table.
    Then I expect value of column "DESC_USAGE_TYP_COUNT" in the below SQL query equals to "1":
      """
       ...
       AND INSTR_ID = (SELECT INSTR_ID FROM FT_T_ISID WHERE ID_CTXT_TYP = 'TMBAMCDE' AND ISS_ID='${ISS_ID}' AND END_TMS IS NULL)
       AND TRUNC(LAST_CHG_TMS)=TRUNC(SYSDATE)
       AND END_TMS IS NULL
      """

  Scenario: TC_6: Data Verifications for 'TMBAM: To verify the ISIN 
  ...
  Scenario: TC_7: Data Verifications for 'TMBAM :To verify the SEDOL
  ...
  Scenario: TC_8: Data Verifications for 'TMBAM:To verify the CUSIP

    Then I expect value of column "CUSIP_COUNT" in the below SQL query equals to "0":
    """
     Select COUNT(*) AS CUSIP_COUNT FROM FT_T_ISID
     WHERE ID_CTXT_TYP = 'CUSIP'
     AND ISS_ID IS NULL AND END_TMS IS NULL
    """

  Scenario: TC_9: Data Verifications for TMBAM:To verify the  Source BU Code
  ...
  Scenario: TC_10: Data Verifications for TMBAM :To verify the  Voting Rights Indicator
  ...
  Scenario: TC_11: Data Verifications for TMBAM:To verify the  Next Conversion Date
    ...
    """
      ...
      WHERE INSTR_ID=(
                      SELECT INSTR_ID FROM FT_T_ISID WHERE ID_CTXT_TYP = 'TMBAMCDE' and ISS_ID='${ISS_ID}' AND END_TMS IS NULL
                      )
      AND NXT_CNVR_PRD_START_DTE IS NULL
      AND TRUNC(LAST_CHG_TMS)=TRUNC(SYSDATE)
    """

  Scenario: TC_12: Data Verifications for TMBAM:To verify the U/L sec ID
    ...
    """
      ...
      INSTR_ID=(SELECT INSTR_ID FROM FT_T_ISID WHERE ID_CTXT_TYP = 'TMBAMCDE' and ISS_ID='${ISS_ID}'
      AND END_TMS IS NULL)
      AND ISS_PART_RL_TYP='UNDLYING'
      AND PART_UNITS_TYP='ALL'
    """

  Scenario: TC_13: Data Verifications for TMBAM:To verify the Expiry Date,Contract Size and Currency of Denomination
    ...
    """
      ...
      INSTR_ID=(SELECT INSTR_ID FROM FT_T_ISID WHERE ID_CTXT_TYP = 'TMBAMCDE' AND ISS_ID='${ISS_ID}' AND END_TMS IS NULL)
      AND MAT_EXP_TMS IS NULL AND ISS_UT_CQTY IS NULL AND DENOM_CURR_CDE='${DENOM_CURR_CODE}'
    """

  Scenario: TC_14: Data Verifications for TMBAM:To verify the CV Terms Numerator and CV Terms Denominator
  ...
  Scenario: TC_15: Data Verifications for TMBAM:To verify the Option type
  ...
  Scenario: TC_16: Data Verifications for M&G:To verify the Security Type/Instrument Type
    ...
    """
      ...
      INDUS_CL_SET_ID = 'TMBAMSCTYP' AND CL_VALUE = '${SEC_TYP}' AND END_TMS IS NULL
    """
