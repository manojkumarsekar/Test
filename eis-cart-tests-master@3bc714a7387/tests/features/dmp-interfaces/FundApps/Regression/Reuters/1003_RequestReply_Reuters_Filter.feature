...
@tom_4347 @tom_4347_1003 @dmp_fundapps_regression @request_reply_reuters @tom_4432 @tom_4458 @tom_4561 @eisdev_6444 @eisdev_6318 @eisdev_6402
Feature: Request reply feature to request securities from Reuters based on positions of the PositionsSource passed to the workflow with Filter flag set to Y.
    ...
    Then I expect value of column "VREQ_STATUS_CHECK" in the below SQL query equals to "PASS":
     """
      ...
      WHERE TRUNC (VND_RQST_TMS) = TRUNC (SYSDATE) AND   VND_RQST_TYP IN ('ReutersDSS_Terms_and_Conditions','EIS_Reuters_Composite') AND   VND_RQST_XREF_ID_CTXT_TYP = 'SED' AND   VND_RQST_XREF_ID IN (
         '6368360','B0XGGY0','BFWB6B8','4354350'
      )
     """
