...
@tom_4347 @tom_4347_1001 @tom_4432 @tom_4458 @tom_4561 @dmp_gs_upgrade @eisdev_6444 @eisdev_6318 @eisdev_6402
Feature: Request reply feature to request securities from Reuters based on positions of the PositionsSource passed to the workflow for Latam.
  ...
  Scenario: TC_1: Create Position file with POS_DATE as SYSDATE and Load into DMP
    ...
    Given I assign "PositionFileLoad_template_latam.xml" to variable "INPUT_TEMPLATENAME"
    ...
    And I execute below query to "Ensure there are no future dated positions (created by other feature files, so rows should be small in number)"
     """
  ...
  Scenario: TC_2: Check Reuters Request Reply for ReutersDSS_Terms_and_Conditions
	...
	VND_RQST_XREF_ID_CTXT_TYP = 'SED' AND VND_RQST_XREF_ID IN ('6368360','B0XGGY0','2491594','BFWB6B8','4354350','BD2NDJ7')
	"""
    ...
    Then I expect value of column "VREQ_STATUS_CHECK" in the below SQL query equals to "PASS":
     """
      ...
      WHERE TRUNC (VND_RQST_TMS) = TRUNC (SYSDATE) AND   VND_RQST_TYP IN ('ReutersDSS_Terms_and_Conditions','EIS_Reuters_Composite') AND   VND_RQST_XREF_ID_CTXT_TYP = 'SED' AND   VND_RQST_XREF_ID IN (
         '6368360','B0XGGY0','2491594','BFWB6B8','4354350','BD2NDJ7'
      )
     """
