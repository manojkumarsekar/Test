...
@eisdev_5396 @sbl @sbl_mng_reload @dmp_fundapps_regression
Feature: FundApps | SBL Positions | MNG | Reload Positions

  This feature file tests the re-load functionality for MNG SBL Positions.
  ...
  Deleting existing positions for T date
    ...
    Given I assign "tests/test-data/dmp-interfaces/FundApps/Functional/SecurityBorrowLending/MNG_Positions" to variable "testdata.path"
    And I assign "BOCI-POSN_TEMPLATE" to variable "INPUT_FILENAME_BEFORE_NORMAL"
    And I assign "BEFORE_MNGEISLSLPOSN" to variable "INPUT_FILENAME_BEFORE_SBL"
    And I assign "AFTER_MNGEISLSLPOSN" to variable "INPUT_FILENAME_AFTER_SBL"
    And I assign "/dmp/archive/in/ssdr/mng" to variable "MOVE_DIRECTORY"
    ...
    """
    DELETE FT_T_BHST WHERE BALH_OID IN (SELECT BALH_OID FROM FT_T_BALH WHERE RQSTR_ID LIKE '%MNG%' AND AS_OF_TMS = TO_DATE ('${DYNAMIC_FILE_DATE}','YYYYMMDD'));
    DELETE FT_T_BALH WHERE RQSTR_ID LIKE '%MNG%' AND AS_OF_TMS = TO_DATE ('${DYNAMIC_FILE_DATE}','YYYYMMDD');
  ...
  Scenario: Load BOCI Normal Position Data
  ...
  Verify 5 Record for BALH are available with RQSTR_ID = 'MNGSBL' AND AS_OF_TMS = TO_DATE ('${DYNAMIC_DATE}','DD/MM/YYYY')

    Then I expect value of column "BALH_MNGSBL_BEFORE" in the below SQL query equals to "3":
    """
    SELECT COUNT(*) AS BALH_MNGSBL_BEFORE FROM FT_T_BALH
    WHERE RQSTR_ID = 'MNGSBL'
  ...
  Verify Workflow is successfully executed with Success Count 2 and Partial Count 1.
    ...
    And I set the workflow template parameter "MESSAGETYPE_GENERATESQLFROMFILE" to "EIS_MT_MNG_DMP_SBL_DELETE_POSITION"
    And I set the workflow template parameter "MESSAGETYPE_STANDARDFILELOAD" to "EIS_MT_MNG_DMP_SBL_POSITION"
  ...
  Verify only 2 Record for BALH with RQSTR_ID = 'MNGSBL' AND AS_OF_TMS = TO_DATE ('${DYNAMIC_DATE}','DD/MM/YYYY') are available

    Then I expect value of column "BALH_MNGSBL_AFTER" in the below SQL query equals to "2":
    """
    SELECT COUNT(*) AS BALH_MNGSBL_AFTER FROM FT_T_BALH
    WHERE RQSTR_ID = 'MNGSBL'
    AND AS_OF_TMS = TO_DATE ('${DYNAMIC_DATE}','DD/MM/YYYY')
    """
  ...
  Scenario: Verify Exception is thrown as part of delete processing for the position date mismatch
    ...
    (SELECT TRN_ID FROM FT_T_TRID WHERE JOB_ID IN
      (SELECT JOB_ID FROM FT_T_JBLG
       ...
       AND JOB_MSG_TYP = 'EIS_MT_MNG_DMP_SBL_DELETE_POSITION')
     AND RECORD_SEQ_NUM = '3')
    AND NOTFCN_ID = '60003'
    AND NOTFCN_STAT_TYP = 'OPEN'
    AND PARM_VAL_TXT like 'User defined Error thrown! . Can not process record as incoming position date 16/02/2020 and prior business date % are different'
    """
  ...
  Scenario: Verify Exception is thrown as part of file load for the position date mismatch
    ...
    (SELECT TRN_ID FROM FT_T_TRID WHERE JOB_ID IN
      (SELECT JOB_ID FROM FT_T_JBLG
       WHERE PRNT_JOB_ID = '${JOB_ID}'
       AND JOB_MSG_TYP = 'EIS_MT_MNG_DMP_SBL_POSITION')
     AND RECORD_SEQ_NUM = '3')
    AND NOTFCN_ID = '60003'
    AND NOTFCN_STAT_TYP = 'OPEN'
    AND PARM_VAL_TXT like 'User defined Error thrown! . Can not process record as incoming position date 16/02/2020 and prior business date % are different'
