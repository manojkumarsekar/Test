...
@dmp_fundapps_regression @tom_4794 @02_multilist_1N2Y
Feature: Test the creation of listing flag for EUR, ASIA and US market group- Two Y flags and One N flag
  ...
  Scenario: Assign variables
    ...
    And I assign "TwoYOneNFlag.csv" to variable "INPUT_FILENAME"
  ...
  Scenario: TC_3: Verify MKT_ID_CTXT_TYP should be 'MIC' for all MKT_OID
    ...
    """
       ...
       WHERE MKT_OID IN
        ...
        AND MKT_ID IN ('CHIJ','SBIJ','SBIU','XXXX','XNGS','SMFF','XTAI')
  ...
  Listing flag should be created  as - EURLST=Y, USLST=Y and ASIALST=N
    ...
    Then I expect value of column "ISST_FLAG_COUNT" in the below SQL query equals to "2":
    """
  ...
  Scenario: Assign variables
    Given I assign "OneNTwoYFlag.csv" to variable "INPUT_FILENAME"
    And I extract below values for row 2 from CSV file "${INPUT_FILENAME}" in local folder "${testdata.path}" with reference to "User Defined Identifier" column and assign to variables:
      | ISIN | ISIN_CODE |
...
#    And  I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${ISIN_CODE}'"
  ...
  Listing flag will be created -  ASIALST=Y, EURLST=Y, and USLST=N
  ...
  Scenario: TC_7: Verify MKT_ID_CTXT_TYP should be 'MIC' for all MKT_OID
    ...
    """
       ...
       WHERE MKT_OID IN
                (SELECT DISTINCT(MKT_OID) FROM FT_T_IMKR WHERE INSTR_ID IN
                     (SELECT INSTR_ID FROM FT_T_ISID WHERE ISS_ID = '${ISIN_CODE}' AND END_TMS IS NULL))
        AND MKT_ID IN ('CHIJ','SBIJ','SBIU','XXXX','XTAI','HSTC','SMBD')
        AND MKT_ID_CTXT_TYP='MIC'
        AND END_TMS IS NULL
    """
  ...
  Listing flag should be created  as - EURLST=Y, USLST=N and ASIALST=Y
    ...
    """
      SELECT COUNT(*) AS ISST_FLAG_COUNT FROM FT_T_ISST
      WHERE STAT_DEF_ID IN('EURLST','ASIALST')
      AND STAT_CHAR_VAL_TXT = 'Y'
      AND INSTR_ID IN (SELECT INSTR_ID FROM FT_T_ISID WHERE ISS_ID = '${ISIN_CODE}' AND END_TMS IS NULL)
      AND DATA_SRC_ID='REUTERS'
      AND END_TMS IS NULL
