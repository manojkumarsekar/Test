...
Feature: TOM-4275: Transaction RCRLBU TMBAM file load (Golden Source)
  ...
  Scenario: TC_1: Clear Trade data and load pre-requisite Org Chart data
    ...
    And I assign "TMBAMEISLTRANSN20190319_test.csv" to variable "INPUT_FILENAME"

    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/TMBAM" with reference to "Security Id" column and assign to variables:
    ...
    And I assign "TMBAMCDE" to variable "ID_CTXT_TYPE"
    And I assign "CRTSID" to variable "ACCT_ID_CTXT"
    And I assign "TMBAMEOD" to variable "TRN_CDE"
  ...
  Scenario: TC_3: File load for RCRLBU Fund for Data Source TMBAM
  ...
  Scenario: TC_5: Load Transaction Data File
    ...
    When I copy files below from local folder "${testdata.path}/inputfiles/TMBAM" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | TMBAMEISLTRANSN20190319_test.csv |

    And I process files with below parameters and wait for the job to be completed
      | FILE_PATTERN  | TMBAMEISLTRANSN*     |
