...
Feature: TOM-4275: Transaction RCRLBU ESGA file load (Golden Source)
  ...
  Scenario: TC_1: Clear Trade data and load pre-requisite Org Chart data
    ...
    And I assign "ESGAEISLTRANSN20190319_test.csv" to variable "INPUT_FILENAME"

    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/ESGA" with reference to "Security ID" column and assign to variables:
      ...
      | Quantity         | TRD_CQT     |
      | Trade CCY        | TRD_CUR_CDE |
      | Trade Price      | TRD_PRZ     |

    And I assign "ESGACODE" to variable "ID_CTXT_TYPE"
    And I assign "CRTSID" to variable "ACCT_ID_CTXT"
    And I assign "ESGAEOD" to variable "TRN_CDE"
  ...
  Scenario: TC_3: File load for RCRLBU Fund for Data Source ESGA
  ...
  Scenario: TC_5: Load Transaction Data File
    ...
    When I copy files below from local folder "${testdata.path}/inputfiles/ESGA" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | ESGAEISLTRANSN20190319_test.csv |

    And I process files with below parameters and wait for the job to be completed
      | FILE_PATTERN  | ESGAEISLTRANSN20190319* |
  ...
  Scenario: TC_6: Verification of EXTR For  Transaction Data from  File
