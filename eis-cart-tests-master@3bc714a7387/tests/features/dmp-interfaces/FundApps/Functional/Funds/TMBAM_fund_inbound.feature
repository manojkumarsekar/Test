...
@dmp_rcrlbu_tmbam_funds @tom_4124 @dmp_fundapps_functional @fund_apps_funds @tom_4489
Feature: TOM-4124: Funds RCRLBU TMBAM file load (Golden Source)
  ...
  Background:
    ...
    And I assign "TBAMEISLFUNDLE20181218_test.csv" to variable "INPUT_FILENAME"
    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/TMBAM" with reference to "Fund Id" column and assign to variables:
      | Fund Id                                        | ACT_ID    |
    ...
    And I assign "TH" to variable "INVLOC"
  ...
  Scenario: TC_2: File load for RCRLBU Fund for Data Source TMBAM
