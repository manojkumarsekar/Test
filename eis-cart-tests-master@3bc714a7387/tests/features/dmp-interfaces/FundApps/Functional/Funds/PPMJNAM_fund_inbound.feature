...
@dmp_rcrlbu_jnam_funds @tom_4124 @dmp_fundapps_functional @fund_apps_funds @tom_4489
Feature: TOM-4124: Funds RCRLBU JNAM file load (Golden Source)
  ...
  Scenario: TC_1: Clear ACID data, set variables and load pre-requisite Org Chart data
    ...
    And I assign "JNAMEISLFUNDLE20181218_test.csv" to variable "INPUT_FILENAME"

    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/JNAM" with reference to "FUND_ID" column and assign to variables:
      | FUND_ID                     | ACT_ID    |
      | FUND_VEHICLE_TYPE           | FVT       |
      | PRU_GROUP_LE_ID             | PRU_LE_ID |
      | INVESTMENT_DISCRETION_LE_ID | INV_LE_ID |

    And I assign "Sole" to variable "LE_VR"
    And I assign "Shared" to variable "LE_INV"
    ...
    And I assign "US" to variable "INVLOC"
    ...
    When I copy files below from local folder "${testdata.path}/inputfiles/JNAM" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | JNAM_ORG_Chart_template.xls |

    And I process files with below parameters and wait for the job to be completed
      | FILE_PATTERN  | JNAM_ORG_Chart_template.xls |
  ...
  Scenario: TC_2: File load for RCRLBU Fund for Data Source JNAM
  ...
  Scenario: TC_8: Verification of LE data
    ...
    #Verification of EXAC for US Custodian Account No for the fund loaded with required data from file
    Then I expect value of column "USCUST_COUNT" in the below SQL query equals to "1":
      """
      SELECT count(*) as USCUST_COUNT FROM FT_T_EXAC
      WHERE END_TMS IS NULL
      AND EXTERNAL_SYS_ACCT_ID = 'SGDM'
      AND EXAC_OID IN (SELECT EXAC_OID FROM FT_T_AEAR WHERE RL_TYP = 'CUSTDIAN' AND ACCT_ID IN (SELECT ACCT_ID FROM FT_T_ACID WHERE ACCT_ALT_ID ='${ACT_ID}' AND ACCT_ID_CTXT_TYP='${ACCT_ID_CTXT}' AND END_TMS IS NULL))
      AND INST_MNEM IN (SELECT INST_MNEM FROM FT_T_FIID WHERE FINS_ID='UNKNOWN' AND FINS_ID_CTXT_TYP='INHOUSE')
      """