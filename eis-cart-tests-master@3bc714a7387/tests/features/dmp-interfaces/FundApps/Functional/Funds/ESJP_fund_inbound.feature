...
@dmp_rcrlbu_esjp_funds @tom_4124 @dmp_fundapps_functional @fund_apps_funds @tom_4489
Feature: TOM-4124: Funds RCRLBU ESJP file load (Golden Source)
  ...
  Scenario: TC_1: Clear ACID data and load pre-requisite Org Chart data
    ...
    And I assign "ESJPEISLFUNDLE20181218_test.csv" to variable "INPUT_FILENAME"

    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/ESJP" with reference to "Fund Id" column and assign to variables:
      | Fund Id                                        | ACT_ID    |
