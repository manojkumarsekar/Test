...
@dmp_rcrlbu_eimk_funds @tom_4124 @dmp_fundapps_functional @fund_apps_funds @tom_4489
Feature: TOM-4124: Funds RCRLBU EIMK file load (Golden Source)
  ...
  Scenario: TC_1: Clear ACID data, set variables and load pre-requisite Org Chart data
    ...
    And I assign "EIMKEISLFUNDLE20181218_test.csv" to variable "INPUT_FILENAME"

    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/EIMK" with reference to "Fund Id" column and assign to variables:
      | Fund Id                                        | ACT_ID    |
    ...
    And I assign "KR" to variable "INVLOC"
  ...
  Scenario: TC_2: File load for RCRLBU Fund for Data Source EIMK
  ...
  Scenario: TC_7: Verification of Currency data (This will be 0 as the data is already set up with other currency)
    #Verification of FNCH for Base Currency for the fund loaded with required data from file
    Then I expect value of column "CURR_COUNT" in the below SQL query equals to "0":
      """
