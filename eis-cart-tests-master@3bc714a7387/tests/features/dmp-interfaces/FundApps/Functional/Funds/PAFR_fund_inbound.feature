#https://jira.pruconnect.net/browse/EISDEV-5217
#https://collaborate.intranet.asia/display/FUNDAPPS/FA-IN-SMF-LBURCR-DMP-Fund-File

@dmp_rcrlbu_pafr_funds @tom_5217 @dmp_fundapps_functional @fund_apps_funds
Feature: TOM-5217: Funds RCRLBU PAFR file load (Golden Source)

  Verifying Fund creation in DMP through new feed Prudential Africa (PAFR) and if the required attributes have been set up.
  ...
  Scenario: TC_1: Clear ACID data and assign variables
    ...
    And I assign "PAFREISLFUNDLE20191031_test.csv" to variable "INPUT_FILENAME"
    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/PAFR" with reference to "Fund Id" column and assign to variables:
      | Fund Id                                        | ACT_ID    |
    ...
    And I assign "ZM" to variable "INVLOC"
  ...
  Scenario: TC_2: File load for RCRLBU Fund for Data Source PAFR

    When I copy files below from local folder "${testdata.path}/inputfiles/PAFR" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | ${INPUT_FILENAME} |
