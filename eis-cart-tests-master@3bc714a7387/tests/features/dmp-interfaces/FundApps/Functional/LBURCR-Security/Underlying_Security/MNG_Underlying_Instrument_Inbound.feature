...
Feature: TOM_4504 SSDR_INBOUND | RCR| LBU Instrument | MANGLBU

  The data points which are common between the files have been verified in the feature file of MANG , this feature file is created to verify data specific to LBU/RCR

  Scenario: TC_1: Clear old test data for MANG and set up variables
    ...
    """
    ${testdata.path}/sql/Clear_data_MNG_Underlying.sql
    """

  Scenario: TC_2: Load MANG file MNG_Security_Underlying.csv

    Given I assign "MNG_Security_Underlying.csv" to variable "MANG_INPUT_FILE"
    Given I assign "tests/test-data/dmp-interfaces/FundApps/Functional/Underlying_security" to variable "testdata.path"
    ...
    Given I process files with below parameters and wait for the job to be completed
      | BUSINESS_FEED | EIS_BF_RCRLBU_DMP_MNG_SECURITY |
      | FILE_PATTERN  | ${MANG_INPUT_FILE}              |
  ...
  Scenario: TC_3:Verifications for Instrument_ID-Check if ISID is created with test file(Security_id,ISIN,SEDOL)
    ...
    Then I expect value of column "VERIFY_ISID_MNG" in the below SQL query equals to "3":
    """
	SELECT
    count(*) as VERIFY_ISID_MNG
	FROM FT_T_ISID
	WHERE INSTR_ID IN
	                  (
	                     SELECT INSTR_ID
	                     FROM FT_T_ISID
	                     WHERE ISS_ID = 'BDD98S8'
	                     AND END_TMS IS NULL
	                  )
    AND ID_CTXT_TYP IN ('ISIN','SEDOL','MNGCODE')
    AND   END_TMS IS NULL
	"""
  ...
  Scenario: TC_4:Verifications for Underlying_id setup with the security id or not
	...
	FROM fT_T_riss WHERE rld_iss_feat_id in
                                             (
                                                 ...
                                                 FROM fT_T_ridf
                                                 WHERE instr_id in (SELECT instr_id FROM ft_T_isid WHERE iss_id='BDD98S8' and end_tms is null)
                                                 and end_tms is null
                                              )

    and instr_id in (SELECT instr_id FROM ft_T_isid WHERE iss_id='6990763' and end_tms is null)
