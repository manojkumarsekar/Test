...
@dmp_rcrlbu_jnam_positions @tom_4127 @dmp_fundapps_functional @fund_apps_positions @tom_4320 @tom_4490
Feature: TOM-4127: Positions RCRLBU JNAM file load (Golden Source)
  ...
  #Prerequisites
  Scenario: TC_1: Clear data for RCRLBU Position for Data Source JNAM and variable assignment

    Given I assign "tests/test-data/dmp-interfaces/FundApps/Functional/Positions" to variable "testdata.path"
    And I assign "JNAMEISLPOSITN20181218_test.csv" to variable "INPUT_FILENAME"

    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/JNAM" with reference to "POSITION_DATE" column and assign to variables:
    ...
    #And I assign "US298785HV15" to variable "INSTRID"
  ...
  Scenario: TC_3: Load pre-requisite Fund Data before file
    ...
    When I copy files below from local folder "${testdata.path}/inputfiles/JNAM" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | JNAMEISLFUNDLE20181218_test.csv |
  ...
  Scenario: TC_4: Load pre-requisite Security Data before file
    ...
    When I copy files below from local folder "${testdata.path}/inputfiles/JNAM" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | JNAMEISLINSTMT20181218_test.csv |
  ...
  Scenario: TC_5: File load for RCRLBU Position for Data Source JNAM
