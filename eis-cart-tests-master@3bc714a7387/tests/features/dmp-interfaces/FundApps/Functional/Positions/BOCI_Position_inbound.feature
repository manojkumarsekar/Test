...
@dmp_gs_upgrade
@dmp_rcrlbu_boci_positions @tom_4127 @dmp_fundapps_functional @fund_apps_positions @tom_4320 @tom_4490
Feature: TOM-4127: Positions RCRLBU BOCI file load (Golden Source)
  ...
  Scenario: TC_1: Clear data for RCRLBU Position for Data Source BOCI and initial variable assignment
    ...
    And I assign "BOCIEISLPOSITN20181218_test.csv" to variable "INPUT_FILENAME"

    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/BOCI" with reference to "POSITION_DATE" column and assign to variables:
    ...
    #And I assign "CNE100001VR3" to variable "INSTRID"
  ...
  Scenario: TC_3: Load pre-requisite Fund Data before file
    ...
    When I copy files below from local folder "${testdata.path}/inputfiles/BOCI" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | BOCIEISLFUNDLE20181218_test.csv |
  ...
  Scenario: TC_4: Load pre-requisite Security Data before file
    ...
    When I copy files below from local folder "${testdata.path}/inputfiles/BOCI" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | BOCIEISLINSTMT20181218_test.csv |
  ...
  Scenario: TC_5: File load for RCRLBU Position for Data Source BOCI
