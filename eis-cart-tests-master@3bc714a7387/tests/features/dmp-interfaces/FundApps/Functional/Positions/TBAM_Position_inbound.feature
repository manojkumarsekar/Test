...
@dmp_rcrlbu_tbam_positions @tom_4127 @dmp_fundapps_functional @fund_apps_positions @tom_4320 @tom_4490
Feature: TOM-4127: Positions RCRLBU TBAM file load (Golden Source)
  ...
  #Prerequisites
  Scenario: TC_1: Clear data for RCRLBU Position for Data Source TMBAM and variable assignment

    Given I assign "tests/test-data/dmp-interfaces/FundApps/Functional/Positions" to variable "testdata.path"
    And I assign "TBAMEISLPOSITN20181218_test.csv" to variable "INPUT_FILENAME"

    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/TMBAM" with reference to "Position Date" column and assign to variables:
    ...
    And I assign "TMBAMCDE" to variable "ISSU_ID_CTXT"
    And I assign "TMBAMEOD" to variable "RQSTR_ID"
    ...
    #And I assign "XS1810024338" to variable "INSTRID"
  ...
  Scenario: TC_2: Load pre-requisite ORG Chart Data before file
    ...
    When I copy files below from local folder "${testdata.path}/inputfiles/TMBAM" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | TMBAM_ORG_Chart_template.xls |

    And I process files with below parameters and wait for the job to be completed
      | FILE_PATTERN  | TMBAM_ORG_Chart_template.xls |
  ...
  Scenario: TC_3: Load pre-requisite Fund Data before file
    ...
    When I copy files below from local folder "${testdata.path}/inputfiles/TMBAM" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | TBAMEISLFUNDLE20181218_test.csv |
  ...
  Scenario: TC_4: Load pre-requisite Security Data before file
    ...
    When I copy files below from local folder "${testdata.path}/inputfiles/TMBAM" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | TBAMEISLINSTMT20181218_test.csv |
  ...
  Scenario: TC_5: File load for RCRLBU Position for Data Source TBAM
