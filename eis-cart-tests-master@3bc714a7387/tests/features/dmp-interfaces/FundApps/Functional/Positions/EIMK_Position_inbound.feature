...
@dmp_rcrlbu_eimk_positions @tom_4127 @dmp_fundapps_functional @fund_apps_positions @tom_4320 @tom_4490
Feature: TOM-4127: Positions RCRLBU EIMK file load (Golden Source)
  ...
  #Prerequisites
  Scenario: TC_1: Clear data for RCRLBU Position for Data Source EIMK and initial variable assignment

    Given I assign "tests/test-data/dmp-interfaces/FundApps/Functional/Positions" to variable "testdata.path"
    And I assign "EIMKEISLPOSITN20181218_test.csv" to variable "INPUT_FILENAME"

    When I extract below values for row 2 from PSV file "${INPUT_FILENAME}" in local folder "${testdata.path}/inputfiles/EIMK" with reference to "Position Date" column and assign to variables:
    ...
    #And I assign "CNE000000040" to variable "INSTRID"
  ...
  Scenario: TC_5: File load for RCRLBU Position for Data Source EMIK
