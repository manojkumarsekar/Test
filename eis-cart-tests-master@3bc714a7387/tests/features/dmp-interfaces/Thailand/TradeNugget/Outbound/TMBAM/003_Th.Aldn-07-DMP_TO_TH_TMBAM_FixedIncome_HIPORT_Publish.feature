...
# Purpose : The purpose of this file to publish on market transactions(FixedIncome) from DMP to TMBAM hiport.
...
@eisdev_6363 @eisdev_6363_tmbam_fi @001_tmbam_onmarket_publish @dmp_thailand_hiport @dmp_thailand @eisdev_6426
@eisdev_6430 @eisdev_6421 @eisdev_5921 @eisdev_6480 @eisdev_6484 @eisdev_6565 @eisdev_6696 @eisdev_6725 @eisdev_6780
@eisdev_6765 @eisdev_6717 @eisdev_6787 @eisdev_6824 @eisdev_6829 @eisdev_6814 @eisdev_6845 @eisdev_7172 @eisdev_7492

Feature: Publish the TMBAM FixedIncome trade in Hiport format
  ...
  SecGroup : BND  | CouponType : Fixed(F)
  Security_ID     | Fund_ID   | Transaction_ID            | Transaction_Type | Transaction_Date |
  IN0020190396    | TMB22     | BHTMBT505${TRD_VAR_NUM_1} | BUY              | ${CURR_DATE_1}   |
  XS1497605805    | TMB22     | BHTMBT606${TRD_VAR_NUM_1} | SELL             | ${CURR_DATE_1}   |

  SecGroup : BND  | CouponType : Floating(T)
  Security_ID     | Fund_ID   | Transaction_ID            | Transaction_Type | Transaction_Date |
  XS0149623281    | TMB22     | BHTMBT707${TRD_VAR_NUM_1} | BUY              | ${CURR_DATE_1}   |
  XS0284753497    | TMB22     | BHTMBT808${TRD_VAR_NUM_1} | SELL             | ${CURR_DATE_1}   |

  SecGroup : BND  | CouponType : Zero Coupon (Z) | It is called Discounted Security
  Security_ID     | Fund_ID   | Transaction_ID            | Transaction_Type | Transaction_Date |
  US912803DM22    | TMB22     | BHTMBT909${TRD_VAR_NUM_1} | BUY              | ${CURR_DATE_1}   |
  US912803FM04    | TMB22     | BHTMT100${TRD_VAR_NUM_1}  | SELL             | ${CURR_DATE_1}   |

  SecGroup : BND  | CouponType : Fixed(F) | Maturity Date = First Pay Date
  Security_ID     | Fund_ID   | Transaction_ID            | Transaction_Type | Transaction_Date |
  BES34WLC9       | TMB22     | BHTMDS11${TRD_VAR_NUM_1}  | BUY              | ${CURR_DATE_1}   |
  BES34WLD7       | TMB22     | BHTMDS22${TRD_VAR_NUM_1}  | SELL             | ${CURR_DATE_1}   |
  BES34WLC9       | TMB22_S   | BHTMDS33${TRD_VAR_NUM_1}  | BUY              | ${CURR_DATE_1}   |
  ...
  Security_ID   | Fund_ID | Transaction_ID            | Transaction_Type | Transaction_Date |
  BES2R9YL3     | TMB22     | BHTMT15${TRD_VAR_NUM_1}   | BUY              | ${CURR_DATE_1}   |
  BES31DZ62     | TMB22     | BHTMT16${TRD_VAR_NUM_1}   | SELL             | ${CURR_DATE_1}   |
  ...
  Scenario:TC1: Initialize all the variables
    ...
    And I assign "003_Th_Aldn-07-DMP_TO_TH_TMBTXNFI_Template.qqq" to variable "PUBLISH_FILE_TEMPLATE"
    And I assign "003_Th_Aldn-07-DMP_TO_TH_TMBTXNFI_Expected" to variable "PUBLISH_FILE_EXPECTED"
    And I assign "003_Th_Aldn-07-DMP_TO_TH_TMBTXNFI_Actual" to variable "PUBLISH_FILE_ACTUAL"
  ...
  Scenario:TC5: Load the Transaction file, it is prerequisite file for OnMarket Publish
    ...
    Then I expect 1 exceptions are captured with the following criteria
      | PARM_VAL_TXT            | TMBAM: HiPort Suffix code (HIPORTSFXCD) is not available for the portfolio TMB22 Trade ID = 11821-BHTMBT011${TRD_VAR_NUM_1} |
      | NOTFCN_ID               | 60036                                                                                                                       |
      | SOURCE_ID               | %_GC%                                                                                                                       |
      | MSG_TYP                 | EIS_MT_BRS_TH_INTRADAY_TRANSACTION                                                                                          |
      | NOTFCN_STAT_TYP         | OPEN                                                                                                                        |
      | MAIN_ENTITY_ID_CTXT_TYP | BRSTRNID-FUND:INVNUM                                                                                                        |
      | MAIN_ENTITY_ID          | TMB22:-BHTMBT011${TRD_VAR_NUM_1}                                                                                            |
      | MSG_SEVERITY_CDE        | 40                                                                                                                          |
  ...
  Scenario:TC6: Verify TH_Hiport_ID rows count for Success and Failure
    ...
    EXEC_TRD_ID IN ( SELECT EXEC_TRD_ID
                     FROM FT_T_EXTR
                     WHERE TRD_ID IN ('11821-BHTMBT505${TRD_VAR_NUM_1}','11821-BHTMBT606${TRD_VAR_NUM_1}',
                                   '11821-BHTMBT707${TRD_VAR_NUM_1}','11821-BHTMBT808${TRD_VAR_NUM_1}',
                                   '11821-BHTMBT909${TRD_VAR_NUM_1}',
                                   '11821-BHTMDS11${TRD_VAR_NUM_1}','11821-BHTMDS22${TRD_VAR_NUM_1}'
                                  )
                   )
    """
