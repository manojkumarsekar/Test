#https://jira.pruconnect.net/browse/EISDEV-6511
...
# Purpose : The purpose of this file to publish on market transactions(SWAP) from DMP to TMBAM hiport.

# EISDEV-6615 Changes --START--
# Using the CUSIP on the transaction file link to the security master file to map to the
# CUSIP_RECEIVE for the receivable leg transaction and CUSIP_PAYABLE for the payable leg transaction.
# EISDEV-6615 Changes --END--
...
# EISDEV-6664 Changes --START--
# OPEN
# PARTIAL/FULL TERMINATION
# EISDEV-6664 Changes --END--
...
# EISDEV-6712 Changes --START--
# Restrict publishing of CSWAP if TRD_REL is not present
# EISDEV-6712 Changes --END--

@gc_interface_trades @gc_interface_portfolios @gc_interface_transactions @gc_interface_securities
@dmp_regression_integrationtest
@eisdev_6511 @eisdev_6511_tmbam_swap @001_tmbam_onmarket_publish @dmp_thailand_hiport @dmp_thailand
@eisdev_6615 @eisdev_6631 @eisdev_6664 @eisdev_6696 @eisdev_6725 @eisdev_6780 @eisdev_6824 @eisdev_6829 @eisdev_6814
@eisdev_6712

Feature: Publish the TMBAM SWAP trade in Hiport format

  This feature will test the below scenarios
  1. Load Portfolio files to create portfolios
  2. Load the Swap security file received as part of the trade nugget
  2. Load the Swap transaction file received as part of the trade nugget
  3. Publish the Swap HiPort file

  SecGroup : SWAP  | SecType : SWAP
  Security_ID   | Fund_ID | Transaction_ID             | Transaction_Type  | Transaction_Date |
  BPM2FWKR8     | TMB22     | S1${TRD_VAR_NUM_1}         | RCVFIX INIT       | ${CURR_DATE_1}   |
  BES2RM3F1     | TMB22     | S2${TRD_VAR_NUM_1}         | PAYFIX INIT       | ${CURR_DATE_1}   |
  BES2QTK50     | TMB22     | S3${TRD_VAR_NUM_1}         | RCVBASE INIT      | ${CURR_DATE_1}   |
  BPM1Q8KU4     | TMB22     | S4${TRD_VAR_NUM_1}         | PAYFIX INIT       | ${CURR_DATE_1}   |
  BPM2GAN68     | TMB22     | S5${TRD_VAR_NUM_1}         | RCVFIX INIT       | ${CURR_DATE_1}   |
  BES2QTK50     | TMB22     | S6${TRD_VAR_NUM_1}         | BUY               | ${CURR_DATE_1}   |
  BPM2GAN68     | TMB22     | S7${TRD_VAR_NUM_1}         | BUY               | ${CURR_DATE_1}   |
  BPM2FWKR8     | TMB22_S   | S8${TRD_VAR_NUM_1}         | RCVFIX INIT       | ${CURR_DATE_1}   |
  ...
  Scenario:TC1: Initialize all the variables
    ...
    #Security Files
    And I assign "012_Th.Aldn-07-DMP_TO_TH_BRS_SWAP_Security_F10_Template.xml" to variable "SECURITY_TEMPLATE"
    And I assign "012_Th.Aldn-07-DMP_TO_TH_BRS_SWAP_Security_F10" to variable "SECURITY_FILE"

    #Transaction Files
    And I assign "012_Th.Aldn-07-DMP_TO_TH_BRS_SWAP_Transaction_F11_Template.xml" to variable "TRANSACTION_TEMPLATE"
    And I assign "012_Th.Aldn-07-DMP_TO_TH_BRS_SWAP_Transaction_F11" to variable "TRANSACTION_FILE"

    #Publish files and directory
    And I assign "012_Th_Aldn-07-DMP_TO_TH_TMBSWAP_Template.qqq" to variable "PUBLISH_FILE_TEMPLATE"
    And I assign "012_Th_Aldn-07-DMP_TO_TH_TMBSWAP_Expected" to variable "PUBLISH_FILE_EXPECTED"
    And I assign "012_Th_Aldn-07-DMP_TO_TH_TMBSWAP_Actual" to variable "PUBLISH_FILE_ACTUAL"
    ...
    And I execute below query and extract values of "TRD_VAR_NUM_1" into same variables
     """
     SELECT SUBSTR(VREQ_FILE_SEQ.NEXTVAL,3) AS TRD_VAR_NUM_1 FROM DUAL
     """
  ...
  Scenario: TC2: Publish any unpublished files to clear the log
    ...
    And I process publishing wrapper with below parameters and wait for the job to be completed
      | PUBLISHING_FILE_NAME        | DummyFile_Swap.qqq                      |
      | SUBSCRIPTION_NAME           | EITH_DMP_TO_TMBAM_HIPORT_TRADE_SWAP_SUB |
      | FOOTER_COUNT                | 1                                       |
  ...
  Scenario:TC6: Publish the Swap file in Hiport format
