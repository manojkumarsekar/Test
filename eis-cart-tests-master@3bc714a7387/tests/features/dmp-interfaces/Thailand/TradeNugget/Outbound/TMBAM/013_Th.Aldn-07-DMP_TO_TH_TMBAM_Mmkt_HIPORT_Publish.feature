...
# Purpose : The purpose of this file to publish MMkt transactions from DMP to TMBAM hiport.

@gc_interface_trades @gc_interface_portfolios @gc_interface_transactions @gc_interface_securities
@dmp_regression_integrationtest
@eisdev_6817 @tmbam_mmkt @012_tmbam_onmarket_publish @dmp_thailand_hiport @dmp_thailand

Feature: Publish the TMBAM Mmkt trade in Hiport format
  ...
  Scenario:TC1: Initialize all the variables
    ...
    #Security Files
    And I assign "013_Th.Aldn-07-DMP_TO_TH_BRS_Mmkt_Security_F10_Template.xml" to variable "SECURITY_TEMPLATE"
    And I assign "013_Th.Aldn-07-DMP_TO_TH_BRS_Mmkt_Security_F10" to variable "SECURITY_FILE"

    #Transaction Files
    And I assign "013_Th.Aldn-07-DMP_TO_TH_BRS_Mmkt_Transaction_F11_Template.xml" to variable "TRANSACTION_TEMPLATE"
    And I assign "013_Th.Aldn-07-DMP_TO_TH_BRS_Mmkt_Transaction_F11" to variable "TRANSACTION_FILE"

    #Publish files and directory
    And I assign "013_Th_Aldn-07-DMP_TO_TH_TMTXNMM01_Template.qqq" to variable "PUBLISH_FILE_01_TEMPLATE"
    And I assign "013_Th_Aldn-07-DMP_TO_TH_TMTXNMM01_Expected" to variable "PUBLISH_FILE_01_EXPECTED"
    And I assign "013_Th_Aldn-07-DMP_TO_TH_TMTXNMM01_Actual" to variable "PUBLISH_FILE_01_ACTUAL"
    ...
    And I execute below query to "clear transactions being loaded in subsequent steps"
     """
     ${testdata.path}/sql/013_Th.Aldn-07-DMP_TO_TH_BRS_Mmkt_Clear_Trades.sql
     """
  ...
  Scenario: TC5: Check if TMBAM Transfer Account Number is updated on the security

    Then I expect value of column "TMTACT_1" in the below SQL query equals to "SAKBANKT":
    """
	SELECT iss_id AS TMTACT_1
	FROM   ft_t_isid
	WHERE  end_tms IS NULL
		AND id_ctxt_typ = 'TMTACT'
		AND instr_id IN (SELECT instr_id
							FROM   ft_t_isid
							WHERE  id_ctxt_typ = 'BCUSIP'
								AND iss_id = 'BES3F80N9'
								AND end_tms IS NULL)
    """
  ...
  Scenario:TC6: Publish Mmkt 01 transaction in Hiport format
    ...
    And I process publishing wrapper with below parameters and wait for the job to be completed
      ...
      | SUBSCRIPTION_NAME           | EITH_DMP_TO_TMBAM_HIPORT_TRADE_MKT01_SUB |
      | FOOTER_COUNT                | 1                                        |
