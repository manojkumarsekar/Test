...
@eisdev_6416 @007_tmbam_onmarket_publish @dmp_thailand_hiport @dmp_thailand @tmbam_hiport @eisdev_6877 @eisdev_6952
Feature: Publish trade to Hiport for a portfolio with CRTSID only
  ...
  Scenario: TC1: Initialize all the variables
    ...
    #Security Files
    And I assign "007_R5.IN-25A BRS_Trade_Nugget_TH_TMBAM_Security_Template.xml" to variable "SECURITY_TEMPLATE"
    And I assign "007_R5.IN-25A BRS_Trade_Nugget_TH_TMBAM_Security_Prerequisite.xml" to variable "SECURITY_FILE"

    #Portfolio Files
    And I assign "007_R5.IN-25A BRS_Trade_Nugget_TH_TMBAM_Portfolio_Template.xml" to variable "PORTFOLIO_TEMPLATE"
    And I assign "007_R5.IN-25A BRS_Trade_Nugget_TH_TMBAM_Portfolio_Prerequisite.xml" to variable "PORTFOLIO_FILE"

    #Transaction Files
    And I assign "007_R5.IN-25A BRS_Trade_Nugget_TH_TMBAM_Transaction_Template.xml" to variable "TRANSACTION_TEMPLATE"
    And I assign "007_R5.IN-25A BRS_Trade_Nugget_TH_TMBAM_Transaction_Prerequisite.xml" to variable "TRANSACTION_FILE"

    #Publish files & directory
    And I assign "007_Trade_Nugget_TH_TMBAM_Transaction_Template_Publish.qqq" to variable "PUBLISH_FILE_TEMPLATE"
    And I assign "007_Trade_Nugget_TH_TMBAM_Transaction_Expected_Publish.qqq" to variable "PUBLISH_FILE_EXPECTED"
    And I assign "007_Trade_Nugget_TH_TMBAM_Transaction_Actual_Publish" to variable "PUBLISH_FILE_ACTUAL"
  ...
  Scenario: TC2: Publish any unpublished files to clear the log
    ...
    Given I process publishing wrapper with below parameters and wait for the job to be completed
      ...
      | SUBSCRIPTION_NAME           | EITH_DMP_TO_TMBAM_HIPORT_TRADE_EQ_SUB |
      | FOOTER_COUNT                | 1                                     |
  ...
  Scenario: TC3: Create the test portfolio and insert portofolio into Portgroup
    ...
    When I process "${testdata.path}/Outbound/TMBAM/inputfiles/testdata/${PORTFOLIO_SETUP_FILE}" file with below parameters
  ...
  Scenario: TC8: Verify the published trade has CRTS ID
    ...
    Given I create input file "${PUBLISH_FILE_EXPECTED}" using template "${PUBLISH_FILE_TEMPLATE}" from location "${testdata.path}/Outbound/TMBAM/outfiles"

    Then I expect all records from file1 of type QQQ exists in file2
      | File1 | ${testdata.path}/Outbound/TMBAM/outfiles/testdata/${PUBLISH_FILE_EXPECTED}                          |
      | File2 | ${testdata.path}/Outbound/TMBAM/outfiles/testdata/${PUBLISH_FILE_ACTUAL}_${DYNAMIC_FILE_DATE}_1.qqq |