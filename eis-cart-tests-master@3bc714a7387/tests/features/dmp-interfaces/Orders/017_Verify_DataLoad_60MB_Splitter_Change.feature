#https://jira.pruconnect.net/servicedesk/customer/portal/1/CISSD-17345
#CISSD-17345: removed from regression suite due to engineering team have limited the file size due to performance issues.
@dmp_orders @tom_3944 @esi_orders_verify_mapping_69mb
Feature: 017 | Orders | Verify Data Mapping

  Scenario: Verify Data Load for message size 60MB
  ...
  # Clear Intraday Automation Orders
    ...
    LAST_CHG_TMS = SYSDATE WHERE PREF_ORDER_ID IN ('A1889676') AND PREF_ORDER_ID_CTXT_TYP = 'BRS_ORDER';
    COMMIT
    """
  ...
  #Assign Variables
    ...
    And I assign "017_esi_orders_60MBmsg.xml" to variable "INPUT_FILENAME"
    And I assign "017_esi_orders_60MBmsg" to variable "PUBLISHING_FILE_NAME"
  ...
  #Load Data
    ...
    And I set the workflow template parameter "FILE_PATTERN" to "017_esi_orders_60MBmsg.xml"
  ...
  #Verify Data
    ...
    # Verify Parent AUOR
    Then I expect value of column "RECORD_COUNT" in the below SQL query equals to "13":
    """
     SELECT COUNT(*) AS RECORD_COUNT FROM FT_T_AUOR WHERE
     PREF_ORDER_ID = 'A1889676'
