...
@tom_3835 @verify_domain
Feature: 016 | Orders | Verify Block Order with multiple allocation of same fund

  This file to test the below domain failure exception for AUOR.
  The following field(s)and value(s)(stored in the Authorized Order table) being supplied by BRS cannot be validated against the list in the External Domain table:Authorized Order Type = O

  Scenario: Verify Domains
  ...
  #Pre-requisite :
    ...
    LAST_CHG_TMS = SYSDATE WHERE PREF_ORDER_ID IN ('AAB17576') AND PREF_ORDER_ID_CTXT_TYP = 'BRS_ORDER';
    COMMIT
    """
  ...
  #Assign Variables
    ...
    And I assign "016_esi_orders.verify_domain.xml" to variable "INPUT_FILENAME"
    And I assign "016_ESI_Orders_Verify_Domain" to variable "PUBLISHING_FILE_NAME"
  ...
  #Verify Data
    ...
    WHERE PREF_ORDER_ID IN ('AAB17576')
