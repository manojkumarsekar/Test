...
@tom_3620 @esi_order_load_multiple_intraday @tom_4090
Feature: 012 | Orders | Load and Publish Multiple Intraday Order File

  Intraday 1
  FUND   | BCUSIP    | ORDER ID| STATUS   | TOUCH_COUNT | SPLIT_FROM
  AHPESF | BRT7MJV79 | AA175776 | ACTIVE  | 1           |
  AHOHKD | BRT7MJV79 | AA175776 | ACTIVE  | 1           |
  APPHBF | BRT7MJV79 | AA175776 | ACTIVE  | 1           |

  Intraday 2
  FUND   | BCUSIP    | ORDER ID| STATUS   | TOUCH_COUNT | SPLIT_FROM
  AHPESF | BRT7MJV79 | AA175776 | ACTIVE  | 2           |
  APPHBF | BRT7MJV79 | AA175776 | ACTIVE  | 2           |
  AHOHKD | BRT7MJV79 | AA177076 | ACTIVE  | 1           | AA175776

  Intraday 3
  FUND   | BCUSIP    | ORDER ID| STATUS   | TOUCH_COUNT | SPLIT_FROM
  AHPESF | BRT7MJV79 | AA175776 | ACTIVE  | 3           |
  APPHBF | BRT7MJV79 | AA178076 | ACTIVE  | 1           | AA175776

  Scenario: Load Multiple Intrday files where to correctly end_date/inactivate relationship in AOAR table for split portfolios/instruments
  ...
  #Pre-requisite :
    ...
    LAST_CHG_TMS = SYSDATE WHERE PREF_ORDER_ID IN ('AA175776','AA177076','AA178076') AND PREF_ORDER_ID_CTXT_TYP = 'BRS_ORDER';
    COMMIT
    """
  ...
  #Assign Variables
    ...
    And I assign "012_esi_orders.Load_Multiple_Files_1.xml" to variable "INPUT_FILENAME"
    And I assign "012_esi_orders.Load_Multiple_Files_2.xml" to variable "INPUT_FILENAME_2"
    And I assign "012_esi_orders.Load_Multiple_Files_3.xml" to variable "INPUT_FILENAME_3"
    And I assign "012_ESI_Orders_Load_Multiple_Files_1" to variable "PUBLISHING_FILE_NAME"
    And I assign "012_ESI_Orders_Load_Multiple_Files_Master_Template.csv" to variable "MASTER_TEMPLATE"
    And I assign "012_ESI_Orders_Load_Multiple_Files_Master.csv" to variable "MASTER_FILE"
  ...
  #Load Data
    ...
    And I set the workflow template parameter "FILE_PATTERN" to "012_esi_orders.Load_Multiple_Files*.xml"
  ...
  #Verify Data
    ...
    FROM FT_T_AUOR AUOR
    WHERE PREF_ORDER_ID IN ('AA175776','AA177076','AA178076')
    ...
    """
    SELECT COUNT(*) AS RECORD_COUNT
    FROM FT_T_AUOR WHERE PREF_ORDER_ID IN ('AA175776','AA178076')
    AND PREF_ORDER_ID_CTXT_TYP = 'BRS_ORDER' AND LAST_CHG_USR_ID = 'BATCHJOB_INACTIVATEORDERS'
    """
    ...
    FROM FT_T_AOAR
    WHERE AUOR_OID IN(
    SELECT AUOR_OID FROM FT_T_AUOR AUOR WHERE PREF_ORDER_ID IN ('AA175776','AA178076')
    AND PREF_ORDER_ID_CTXT_TYP = 'BRS_ORDER' AND LAST_CHG_USR_ID = 'BATCHJOB_INACTIVATEORDERS')
