...
@dmp_dw_regression
@tom_4971 @tom_4971_update @perf_attrib_l3 @eisdev_5525 @eisdev_5564
Feature: Test BNP performance L3 file for update

  This is to test if BNP L3 performance file is getting loaded in DWH for update - versioning test
  ...
  Scenario: TC_1: Clear old test data and setup variables
    ...
    """
     DELETE ft_t_wpea WHERE acct_sok_1 = (select acct_sok from ft_t_wact where intrnl_id10 = 'ALAIEFTST' and  DW_STATUS_NUM =1);
    """
  ...
  Scenario: TC_2: Load BNP L3 performance file to insert record
  ...
  Scenario: TC_2: Load BNP L3 performance file to update the same record

    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | 1002_PERF_UPDATE_L3.csv |

    And I process files with below parameters and wait for the job to be completed
      | BUSINESS_FEED |                               |
      | FILE_PATTERN  | 1002_PERF_UPDATE_L3.csv       |
      | MESSAGE_TYPE  | EIS_MT_BNP_DWH_PERF_ATTRIB_L3 |
  ...
  Scenario: TC_3: Perform checks in WPEA for multiple scenarios
    ...
    Then I expect value of column "WPEA_CORRECTEDREC_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WPEA_CORRECTEDREC_COUNT
      FROM FT_T_WPEA WHERE ACCT_SOK_1 = (select acct_sok from ft_t_wact where intrnl_id10 = 'ALAIEFTST' and  DW_STATUS_NUM =1)
      and DW_STATUS_NUM =1
      """

    Then I expect value of column "WPEA_INACTIVEREC_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WPEA_INACTIVEREC_COUNT
      FROM FT_T_WPEA WHERE ACCT_SOK_1 = (select acct_sok from ft_t_wact where intrnl_id10 = 'ALAIEFTST' and  DW_STATUS_NUM =1)
      and DW_STATUS_NUM =0
      """
