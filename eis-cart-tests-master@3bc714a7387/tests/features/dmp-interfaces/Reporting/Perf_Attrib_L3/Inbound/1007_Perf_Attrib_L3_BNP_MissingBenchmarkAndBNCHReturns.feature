...
@dmp_dw_regression
@tom_4971 @tom_4971_missingbnchreturns @perf_attrib_l3 @eisdev_5525
Feature: Test BNP performance L3 file for missing benchmark and returns

  This is to test if BNP L3 performance file is not throwing error for missing portfolio and missing benchmark returns
  ...
  Scenario: TC_2: Load BNP L3 performance file
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | 1008_PERF_INSERT_L3.csv |

    And I process files with below parameters and wait for the job to be completed
      | BUSINESS_FEED |                               |
      | FILE_PATTERN  | 1008_PERF_INSERT_L3.csv       |
  ...
  Scenario: TC_3: Verify if the exception is thrown
    ...
    Then I expect value of column "NTEL_COUNT" in the below SQL query equals to "0":
      ...
      AND MAIN_ENTITY_ID = '31519:30619:US3168271043:CashandEquivalents:ALAIEFTST:COUNTRY POCKET:1:CashandEquivalents:SubTotal:'
      AND MAIN_ENTITY_ID_CTXT_TYP = 'IRP:ISIN:BRK:DP:LVL'
      AND PARM_VAL_TXT = 'BL3PRIM IRPID ALAIEFTST EIS GSWDateDefinition'
