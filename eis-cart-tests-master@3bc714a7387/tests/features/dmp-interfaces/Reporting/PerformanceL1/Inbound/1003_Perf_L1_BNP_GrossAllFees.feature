...
# 12/11/2019      TOM-5237    Remaining L1 Data fields to be exposed in CORIC Outbound files (Removed Secondary Benchmark from the input file hence counts changed)
...
@dmp_dw_regression
@tom_4930 @tom_4930_GrossAllFees @tom_5143 @perf_l1 @eisdev_7166 @eisdev_5173
Feature: Test BNP performance L1 file for return type Gross of all fees
  ...
  Scenario: TC_2: Load BNP L1 performance file
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | PERF_L1_GrossAllFees.csv |

    And I process files with below parameters and wait for the job to be completed
      | BUSINESS_FEED |                          |
      | FILE_PATTERN  | PERF_L1_GrossAllFees.csv |
  ...
  Scenario: TC_3: Perform checks in WCRI for multiple scenarios for account returns
    ...
    Then I expect value of column "WCRI_GrossAllFees_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WCRI_GrossAllFees_COUNT
      FROM FT_T_WCRI WHERE ACCT_SOK = (select acct_sok from ft_t_wact where intrnl_id10 = 'AGOCABTST' and  DW_STATUS_NUM =1)
      and return_typ = 'GrossAllFees' and DW_STATUS_NUM =1
      """

    Then I expect value of column "WCRI_NetNAVGrossAllFees_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WCRI_NetNAVGrossAllFees_COUNT
      FROM FT_T_WCRI WHERE ACCT_SOK = (select acct_sok from ft_t_wact where intrnl_id10 = 'AGOCABTST' and  DW_STATUS_NUM =1)
      and return_typ = 'NetNAVGrossAllFees' and DW_STATUS_NUM =1
      """

    Then I expect value of column "WCRI_NetSalesChargeNAVGrossAllFees_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WCRI_NetSalesChargeNAVGrossAllFees_COUNT
      FROM FT_T_WCRI WHERE ACCT_SOK = (select acct_sok from ft_t_wact where intrnl_id10 = 'AGOCABTST' and  DW_STATUS_NUM =1)
      and return_typ = 'NetSalesChargeNAVGrossAllFees' and DW_STATUS_NUM =1
      """
