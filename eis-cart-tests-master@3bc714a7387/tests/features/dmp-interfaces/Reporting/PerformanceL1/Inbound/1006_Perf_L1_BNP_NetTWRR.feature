...
@dmp_dw_regression
@tom_4930 @tom_4930_NetTWRR @tom_5143 @perf_l1 @eisdev_7166 @eisdev_5173
Feature: Test BNP performance L1 file for return type Net TWRR

  This is to test if BNP L1 performance file is getting loaded in DWH for return type Net TWRR and its gross columns are stored correctly
  ...
  Scenario: TC_2: Load BNP L1 performance file
    ...
    And I process files with below parameters and wait for the job to be completed
      | BUSINESS_FEED |                        |
      | FILE_PATTERN  | PERF_L1_NetTWRR.csv    |
  ...
  Scenario: TC_3: Perform checks in WCRI for multiple scenarios for account returns
    ...
    Then I expect value of column "WCRI_NetTWRR_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WCRI_NetTWRR_COUNT
      FROM FT_T_WCRI WHERE ACCT_SOK = (select acct_sok from ft_t_wact where intrnl_id10 = 'AGOCABTST' and  DW_STATUS_NUM =1)
      and return_typ = 'NetTWRR' and DW_STATUS_NUM =1
      """

    Then I expect value of column "WCRI_NetSalesChargeNetTWRR_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WCRI_NetSalesChargeNetTWRR_COUNT
      FROM FT_T_WCRI WHERE ACCT_SOK = (select acct_sok from ft_t_wact where intrnl_id10 = 'AGOCABTST' and  DW_STATUS_NUM =1)
      and return_typ = 'NetSalesChargeNetTWRR' and DW_STATUS_NUM =1
      """

    Then I expect value of column "WCRI_GrossMgtFeesNetTWRR_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WCRI_GrossMgtFeesNetTWRR_COUNT
      FROM FT_T_WCRI WHERE ACCT_SOK = (select acct_sok from ft_t_wact where intrnl_id10 = 'AGOCABTST' and  DW_STATUS_NUM =1)
      and return_typ = 'GrossMgtFeesNetTWRR' and DW_STATUS_NUM =1
      """