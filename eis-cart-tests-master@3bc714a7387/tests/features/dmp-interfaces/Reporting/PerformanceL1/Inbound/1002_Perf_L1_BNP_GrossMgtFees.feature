...
@dmp_dw_regression
@tom_4930 @tom_4930_GrossMgtFees @tom_5143 @perf_l1 @eisdev_7166 @eisdev_5173
Feature: Test BNP performance L1 file for return type Gross (Mgt Fee)

  This is to test if BNP L1 performance file is getting loaded in DWH for return type Gross (Mgt Fee) and its net columns are stored correctly
  ...
  Scenario: TC_2: Load BNP L1 performance file
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${dmp.ssh.inbound.path}":
      | PERF_L1_GrossMgtFees.csv |

    And I process files with below parameters and wait for the job to be completed
      | BUSINESS_FEED |                          |
      | FILE_PATTERN  | PERF_L1_GrossMgtFees.csv |
  ...
  Scenario: TC_3: Perform checks in WCRI for multiple scenarios for account returns
    ...
    Then I expect value of column "WCRI_GrossMgtFees_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WCRI_GrossMgtFees_COUNT
      FROM FT_T_WCRI WHERE ACCT_SOK = (select acct_sok from ft_t_wact where intrnl_id10 = 'AGOCABTST' and  DW_STATUS_NUM =1)
      and return_typ = 'GrossMgtFees' and DW_STATUS_NUM =1
      """

    Then I expect value of column "WCRI_NetNAVGrossMgtFees_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WCRI_NetNAVGrossMgtFees_COUNT
      FROM FT_T_WCRI WHERE ACCT_SOK = (select acct_sok from ft_t_wact where intrnl_id10 = 'AGOCABTST' and  DW_STATUS_NUM =1)
      and return_typ = 'NetNAVGrossMgtFees' and DW_STATUS_NUM =1
      """

    Then I expect value of column "WCRI_NetSalesChargeNAVGrossMgtFees_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS WCRI_NetSalesChargeNAVGrossMgtFees_COUNT
      FROM FT_T_WCRI WHERE ACCT_SOK = (select acct_sok from ft_t_wact where intrnl_id10 = 'AGOCABTST' and  DW_STATUS_NUM =1)
      and return_typ = 'NetSalesChargeNAVGrossMgtFees' and DW_STATUS_NUM =1
      """
