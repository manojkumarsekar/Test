...
@eisdev_6851_ISIN_XLS @eisdev_6851 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load JP Unit Trust Morningstar XLS report with incorrect/invalid ISIN and verify exceptions

  This is to test if JP Unit Trust MorningStar Peer Ranking XLS files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_Performance_Comparison_JP_ISIN_20200914_Local_PR-JP_20200914.csv" to variable "OUTPUT_FILENAME_LOCALPRJP"
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Performance Comparison_JP_ISIN_20200914.xls |
