...
@eisdev_6851_XLS @eisdev_6851 @peer_ranking_dwh @eisdev_7414
Feature: Load and Update JP Unit Trust Morningstar XLS report and verify details

  This is to test if JP Unit Trust MorningStar Peer Ranking XLS files are getting converted to CSV format and getting loaded in DWH
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_Performance_Comparison_JP_Load_20200914_Local_PR-JP_20200914.csv" to variable "OUTPUT_FILENAME_LOCALPRJP"
    And I assign "Processed_Performance_Comparison_JP_Update_20200914_Local_PR-JP_20200914.csv" to variable "OUTPUT_FILENAME_UPDATE_LOCALPRJP"
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Performance Comparison_JP_Load_20200914.xls |
  ...
  Scenario: Perform checks in WCRI for checking load scenario
    ...
    """
      ...
      WHERE DATA_SRC_ID = 'MSJPUT' AND DW_STATUS_NUM =1 and ACCT_SOK = (select acct_sok from FT_T_WAIR where iss_sok in (select iss_sok from ft_t_wisu where isin_id ='TW000T0723Y4' and dw_Status_num =1 and PREF_ID_CTXT_TYP='EISSECID') and dw_Status_num =1 and ACCT_ISSU_RL_TYP='AUT')
    """
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow for update file
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Performance Comparison_JP_Update_20200914.xls |
