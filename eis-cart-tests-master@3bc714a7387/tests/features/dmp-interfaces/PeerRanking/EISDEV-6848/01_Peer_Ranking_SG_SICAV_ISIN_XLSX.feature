...
@eisdev_6848_ISIN_XLSX @eisdev_6848 @peer_ranking_dwh @eisdev_7414
Feature: Load SG SICAV Morningstar XLSX report with incorrect/invalid ISIN and verify exceptions

  This is to test if SG SICAV MorningStar Peer Ranking XLSX files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_SICAV_Mtking_Peer_Ranking_ISIN_ES_20200908_SICAV_M_PR_-_All_Funds1_ES_20200908.csv" to variable "OUTPUT_FILENAME_SGSICAV"
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | SICAV Mtking Peer Ranking ISIN (ES)_20200908.xlsx |
