...
@eisdev_6963_ISIN_XLS @eisdev_6963 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load KOR PER XLS report with incorrect/invalid ISIN and verify exceptions

  This is to test if KOR PER Peer Ranking XLS files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | ISIN Fund performance as of 202008.xls |
