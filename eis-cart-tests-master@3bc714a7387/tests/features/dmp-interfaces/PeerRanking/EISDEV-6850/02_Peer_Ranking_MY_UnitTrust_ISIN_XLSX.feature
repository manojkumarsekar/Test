...
@eisdev_6850_ISIN_XLSX @eisdev_6850 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load MY Unit Trust Morningstar XLSX report with incorrect/invalid ISIN and verify exceptions

  This is to test if MY Unit Trust MorningStar Peer Ranking XLSX files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_Malaysia_Peer_Ranking_Template_ISIN_20200909_Msia_UT_Non_Shariah_and_Shariah_20200909.csv" to variable "OUTPUT_FILENAME_MY"
  ...
  Scenario: Clear old test data and setup variables
    ...
    Given I assign "The Issue for 'ISIN - ABC00T0723Y2' provided by EIS is not present in the GSWWISKIssue." to variable "PERF_PUBLISHING_ERRORMMY"
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Malaysia Peer Ranking Template_ISIN_20200909.xlsx |
