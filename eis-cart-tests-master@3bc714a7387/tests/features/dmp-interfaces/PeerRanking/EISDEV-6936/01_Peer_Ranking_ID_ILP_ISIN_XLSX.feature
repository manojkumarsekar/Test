...
@eisdev_6936_ISIN_XLSX @eisdev_6936 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load ID ILP Morningstar XLSX report with incorrect/invalid ISIN and verify exceptions

  This is to test if ID ILP MorningStar Peer Ranking XLSX files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Peers Ranking for Regional ISIN - October 2020.xlsx |
