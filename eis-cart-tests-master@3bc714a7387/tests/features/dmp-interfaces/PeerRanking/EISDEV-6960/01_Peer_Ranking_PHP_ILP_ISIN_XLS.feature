...
@eisdev_6960_ISIN_XLS @eisdev_6960 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load PHP ILP Morningstar XLS report with incorrect/invalid ISIN and verify exceptions

  This is to test if PHP ILP MorningStar Peer Ranking XLS files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_Peer_Rank_ISIN_202008_-_PHP_ILP_ExtFunds_Load_File.csv" to variable "OUTPUT_FILENAME_PHPILP"
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Peer Rank ISIN 202008 - PHP ILP (ExtFunds).xls |
