...
@eisdev_6852_ISIN_XLS @eisdev_6852 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load TW Unit Trust Morningstar XLS report with incorrect/invalid ISIN and verify exceptions

  This is to test if TW Unit Trust MorningStar Peer Ranking XLS files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_Performance_Comparison_TW_ISIN_20200914_Local_PR-TW_20200914.csv" to variable "OUTPUT_FILENAME_LOCALPRTW"
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Performance Comparison_TW_ISIN_20200914.xls |
