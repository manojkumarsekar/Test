...
@eisdev_6964_ISIN_XLSX @eisdev_6964 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load KOR LBU XLSX report with incorrect/invalid ISIN and verify exceptions

  This is to test if KOR LBU Peer Ranking XLSX files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_Peer_group_rankingE_ISIN__20200831_Equity.csv" to variable "OUTPUT_FILENAME_KORLBU"
  ...
  Scenario: Clear old test data and setup variables
    ...
    """
      UPDATE ft_t_wcri SET DW_STATUS_NUM=0,VERSION_END_TMSMP = TO_DATE('${VAR_SYSDATE}','YYYYMMDD-HH24MISS') WHERE DATA_SRC_ID = 'MSKORLBU' AND DW_STATUS_NUM =1 and ACCT_SOK = (select acct_sok from ft_t_wact where dw_status_num =1 and intrnl_id11= 'ABC00T0723Y1');
    """
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Peer group ranking(E) ISIN _20200831.xlsx |
