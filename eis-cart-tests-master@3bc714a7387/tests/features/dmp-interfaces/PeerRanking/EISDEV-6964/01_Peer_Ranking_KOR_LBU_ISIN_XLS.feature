...
@eisdev_6964_ISIN_XLS @eisdev_6964 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load KOR LBU XLS report with incorrect/invalid ISIN and verify exceptions

  This is to test if KOR LBU Peer Ranking XLS files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Peer group ranking(E) ISIN _20200831.xls |
