...
@eisdev_6935_ISIN_XLSX @eisdev_6935 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load MY ILP Morningstar XLSX report with incorrect/invalid ISIN and verify exceptions

  This is to test if MY ILP MorningStar Peer Ranking XLSX files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_202009_Malaysia_Peer_Ranking_Insurance_Fund_ISIN_Load_file.csv" to variable "OUTPUT_FILENAME_MYILP"
  ...
  Scenario: Clear old test data and setup variables
    ...
    Given I assign "The Account Alternate Identifier 'IRPID - MYGEQF21234' received from EIS  could not be retrieved from the GSWWACKAccount" to variable "PERF_PUBLISHING_ERROR"
    And I execute below query
    """
      UPDATE ft_t_wcri SET DW_STATUS_NUM=0,VERSION_END_TMSMP = TO_DATE('${VAR_SYSDATE}','YYYYMMDD-HH24MISS') WHERE DATA_SRC_ID = 'MSMYILP' AND DW_STATUS_NUM =1 and ACCT_SOK = (select acct_sok from ft_t_wact where dw_status_num =1 and intrnl_id10= 'MYGEQF21234');
    """
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | 202009 Malaysia Peer Ranking Insurance Fund ISIN.xlsx |
  ...
  Scenario: Verify if the exception is thrown
    ...
    """
      ...
      AND MAIN_ENTITY_ID LIKE 'MYGEQF21234%'
    ...
    """
      ...
      AND PART_ID = 'NESTED'
      AND MSG_SEVERITY_CDE = 40
      AND MAIN_ENTITY_ID = 'MYGEQF21234::08-31-2020 12:00:00 AM'
      AND MAIN_ENTITY_ID_CTXT_TYP = 'IRP:StrtDte:EndDte'
    """