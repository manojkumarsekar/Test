...
@eisdev_6935_ISIN_XLS @eisdev_6935 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load MY ILP Morningstar XLS report with incorrect/invalid ISIN and verify exceptions

  This is to test if MY ILP MorningStar Peer Ranking XLS files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_202008_Malaysia_Peer_Ranking_Insurance_Fund_ISIN_Load_file.csv" to variable "OUTPUT_FILENAME_MYILP"
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | 202008 Malaysia Peer Ranking Insurance Fund ISIN.xls |
