...
@eisdev_6849_isin_XLS @eisdev_6849 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load SG Unit Trust Morningstar XLS report with incorrect/invalid ISIN and verify exceptions

  This is to test if SG Unit Trust MorningStar Peer Ranking XLS files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_Performance_Comparison_SG_ISIN_20200914_Local_PR-SG_20200914.csv" to variable "OUTPUT_FILENAME_LOCALPRSG"
  ...
  Scenario: Clear old test data and setup variables
    ...
    Given I assign "The Issue for 'ISIN - ABC00T0723Y1' provided by EIS is not present in the GSWWISKIssue." to variable "PERF_PUBLISHING_ERRORMSG"
    ...
    """
      ...
      UPDATE fT_T_wagp SET DW_STATUS_NUM=0,VERSION_END_TMSMP = TO_DATE('${VAR_SYSDATE}','YYYYMMDD-HH24MISS') where DATA_SRC_ID = 'MSSGUT' AND ACCT_PRT_PURP_TYP='PRRNKPCT' and dw_Status_num =1 and prnt_wagr_sok in (select wagr_sok from ft_T_wagr where dw_Status_num =1 and ACCT_GRP_PURP_TYP='PRRNKGR' and ACCT_GRP_DESC ='Japan Fund Japan Small/Mid-Cap Value Equ');
      UPDATE fT_T_wagr SET DW_STATUS_NUM=0,VERSION_END_TMSMP = TO_DATE('${VAR_SYSDATE}','YYYYMMDD-HH24MISS') where DATA_SRC_ID = 'MSSGUT' AND dw_Status_num =1 and ACCT_GRP_PURP_TYP='PRRNKGR' and ACCT_GRP_DESC ='Japan Fund Japan Small/Mid-Cap Value Equ';
    """
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Performance Comparison_SG_ISIN_20200914.xls |
