...
@eisdev_6849_ISIN_XLSX @eisdev_6849 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load SG Unit Trust Morningstar XLSX report with incorrect/invalid ISIN and verify exceptions

  This is to test if SG Unit Trust MorningStar Peer Ranking XLSX files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_Performance_Comparison_SG_ISIN_20200915_Local_PR-SG_20200915.csv" to variable "OUTPUT_FILENAME_LOCALPRSG"
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Performance Comparison_SG_ISIN_20200915.xlsx |
