...
@eisdev_6853_ISIN_XLS @eisdev_6853 @peer_ranking_dwh @eisdev_7214 @eisdev_7414
Feature: Load SG ILP Morningstar XLS report with incorrect/invalid ISIN and verify exceptions

  This is to test if SG ILP MorningStar Peer Ranking XLS files with incorrect/invalid ISIN are getting converted to CSV format
  ...
  Scenario: Assign Variables
    ...
    And I assign "Processed_Sing_ILP_Ranking_ISIN_202008_Summary_PACS_202008.csv" to variable "OUTPUT_FILENAME_SGILP"
  ...
  Scenario: Run Excel to CSV Peer Ranking workflow
    ...
    Given I copy files below from local folder "${testdata.path}/testdata" to the host "dmp.ssh.inbound" folder "${PEERRANK_DOWNLOAD_DIR}":
      | Sing ILP Ranking ISIN 202008.xls |
