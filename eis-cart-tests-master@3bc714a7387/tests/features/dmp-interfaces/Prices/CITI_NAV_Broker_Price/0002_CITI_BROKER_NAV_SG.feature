...
# ===================================================================================================================================================================================
# 17/8/2020     | 10.914   | PBFD    | record should publish successfully           | Record should be translated      | No error     | Record should be published
# 17/8/2020     | 10.914   |         | Missing Fund code                            | Record should not be translated  | Exception Should be Thrown     | Record should not be published
# 17/8/2020     |          | PBFD    | Missing nav                                  | Record should be translated      | Exception Should be Thrown     | Record should not be published
# 17/8/2020     | 10.914   | PBFDTEST| record should not publish                    | Record should be translated      | Exception Should be Thrown     | Record should not be published
# Feature file to test to load data from CITI SG Price and publish data from DMP to BRS.
#https://jira.pruconnect.net/browse/EISDEV-7170
#EXM Rel 6 - Removing scenarios for exception validations with Blank CLIENT_ID

@eisdev_6419 @dmp_interfaces @dmp_regression_integrationtest @dmp_prices @gc_interface_excel2csv @citi_price_sg @manual_uploader @eisdev_7170
Feature: 002 | Price | CITI Broker Price SG | Verify Price Load/Publish
  ...
  Scenario: Transform CITI SG broker price file to XML format
    ...
    And I assign "CITI_SG.xls" to variable "INPUT_FILENAME"
    And I assign "esisg_dmp_citi_broker_price_sg" to variable "TRANSFORMED_FILE_NAME"
    ...
    And I assign "esi_brs_citi_int_price_sg_template.csv" to variable "TEMPLATE_FILENAME"
    ...
    And I process the workflow template file "${XLSX_TO_CSV_WF}" with below parameters and wait for the job to be completed
      | MESSAGE_TYPE    | EIS_MT_CITI_BROKER_PRICE_SG    |
