...
@tom_4095 @tom_4095_006_canctrade @tom_4476 @tw_settlement_instruction_format
@eisdev_7439

Feature: Test SSI report for cancel trade
  ...
  Scenario: Clear table data and setup variables
    ...
    """
     ...
     WHERE TRD_ID IN ('4267-SITEST6') AND END_TMS IS NULL;
     COMMIT
    """
    ...
    WHERE EXEC_TRN_ID IN ('4267-SITEST6')
  ...
  Scenario: Load portfolio, security, broker and trade file
    ...
    And I assign "006_new_trade_CIS.xml" to variable "INPUT_FILENAME3"
  ...
  Scenario: Performing verifications
    ...
    """
      select count(*) TRANS_ROW_COUNT from ft_t_extr where trd_id ='4267-SITEST6' and end_tms is null
    """
    ...
    """
      ...
      WHERE TRD_ID = '4267-SITEST6' AND  END_TMS IS NULL
      )
    """
  ...
  Scenario:  Load Trades file with cancel status

    Given I assign "006_canc_trade_CIS.xml" to variable "INPUT_FILENAME"
  ...
  Scenario: Run publish document workflow for trade
    ...
    """
      SELECT COUNT(*) AS EXST_EIS_PROCESSED_ROW_COUNT FROM FT_T_EXST EXST
      WHERE EXST.EXEC_TRD_STAT_TYP = 'CANCSENT'
      AND EXST.GEN_CNT = (SELECT MAX (GEN_CNT) FROM FT_T_EXST EXST1 WHERE EXST1.EXEC_TRD_ID = EXST.EXEC_TRD_ID AND EXST1.DATA_SRC_ID= 'EIS')
      AND EXST.DATA_SRC_ID = 'EIS'
      AND EXST.EXEC_TRD_ID IN ( SELECT EXEC_TRD_ID FROM FT_T_EXTR
      WHERE TRD_ID = '4267-SITEST6' AND  END_TMS IS NULL
