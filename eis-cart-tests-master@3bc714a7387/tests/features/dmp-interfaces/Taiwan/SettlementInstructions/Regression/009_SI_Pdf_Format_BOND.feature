...
@tom_4085 @taiwan_settlement_instruction @si_bond_pdfcomparison @si_pdf_format_check
Feature: Load new and Amended Confirmed Trade file in DMP and test SSI report generated for BOND based on requirement
  ...
  Scenario Outline: validate SSI report for BOND/GOVT trade <SSIScenario>
    ...
    Examples: Trade Parameters
      | SSIScenario                                                | Cusip     | Isin     | TranType | TradeDate   | SettleDate  | TrdPortfolio | TrdOriginalFace | TrdCounterParty | Trdprincipal | TrdPrice | TrdCommission | TrdOtherFee | TrdCurrency | TrdCoupon | TrdInterest | TrdCouponType | TrdTouchCount | ReportStatus | ReportName              | TextToSkip          |
      | Confirmed New Trade                                        | ${CUSIP1} | ${ISIN1} | BUY      | ${VAR_DATE} | ${VAR_DATE} | TSTTT56      | 1000            | DBS-TW          | 14000        | 14       | 0             | 1.25        | TWD         | 2         |             | F             | 1             | NEWSENT      | Bond_New_Trade.pdf      | 2019/05/29 11:05:19 |
      | Updated Confirmed New Trade mandatory field(TrdCommission) | ${CUSIP1} | ${ISIN1} | BUY      | ${VAR_DATE} | ${VAR_DATE} | TSTTT56      | 1000            | DBS-TW          | 14000        | 14       | 100           | 1.25        | TWD         | 2         |             | F             | 2             | REVSENT      | Bond_Rev_Commission.pdf | 2019/05/29 11:05:28 |
