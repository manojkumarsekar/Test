...
@tom_4085 @taiwan_settlement_instruction @si_etf_pdfcomparison @si_pdf_format_check
Feature: Load new and Amended Confirmed Trade file in DMP and test SSI report generated for ETF trades based on requirement
 ...
 #Verify Data
    ...
    Examples: Trade Parameters
      | SSIScenario                                                | Cusip     | Isin     | TranType | TradeDate   | SettleDate  | TrdPortfolio | TrdOriginalFace | TrdCounterParty | Trdprincipal | TrdPrice | TrdCommission | TrdOtherFee | TrdCurrency | TrdTouchCount | ReportStatus | ReportName             | TextToSkip          |
      | Confirmed New Trade                                        | ${CUSIP1} | ${ISIN1} | BUY      | ${VAR_DATE} | ${VAR_DATE} | TSTTT56      | 1000            | DBS-TW          | 14000        | 14       | 0             | 1.25        | TWD         | 1             | NEWSENT      | ETF_New_Trade.pdf      | 2019/05/29 10:05:10 |
      | Updated Confirmed New Trade mandatory field(TrdCommission) | ${CUSIP1} | ${ISIN1} | BUY      | ${VAR_DATE} | ${VAR_DATE} | TSTTT56      | 1000            | DBS-TW          | 14000        | 14       | 100           | 1.25        | TWD         | 2             | REVSENT      | ETF_Rev_Commission.pdf | 2019/05/29 10:05:23 |
