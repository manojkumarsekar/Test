...
@tom_4095 @tom_4095_007_nochangestatus @tom_4476 @tw_settlement_instruction_format
@eisdev_7439

Feature: Test SSI report for nochange status
  ...
  Scenario: Clear table data and setup variables
    ...
    """
     ...
     WHERE TRD_ID IN ('4267-SITEST7') AND END_TMS IS NULL;
     COMMIT
    """
    ...
    """
     ...
     WHERE EXEC_TRN_ID IN ('4267-SITEST7')
  ...
  Scenario: Load portfolio, security, broker and trade file
    ...
    And I assign "007_canc_trade_CIS.xml" to variable "INPUT_FILENAME3"
    ...
    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "/dmp/out/taiwan/settlement" if exists:
      | 14022019_${PORTFOLIOCRTSID}_FUND_OPEN_END_*_CANC*.pdf |

    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "/dmp/out/taiwan/settlement" if exists:
      | 14022019_${PORTFOLIOCRTSID}_FUND_OPEN_END_*_CANC*.pdf.error |
  ...
  Scenario: Performing verifications
    ...
    """
      select count(*) TRANS_ROW_COUNT from ft_t_extr where trd_id ='4267-SITEST7' and end_tms is null
    """
    ...
    """
      ...
      WHERE EXST.EXEC_TRD_STAT_TYP = 'CANC'
      ...
      WHERE TRD_ID = '4267-SITEST7' AND  END_TMS IS NULL
      )
    """
  ...
  Scenario: Run publish document workflow for trade
    ...
    Then I expect value of column "NOCHGTRADETRIDCOUNT" in the below SQL query equals to "1":
    """
    SELECT COUNT(*) AS NOCHGTRADETRIDCOUNT FROM FT_T_TRID
    WHERE MAIN_ENTITY_ID = (SELECT EXEC_TRD_ID FROM FT_T_EXTR WHERE TRD_ID = '4267-SITEST7' AND  END_TMS IS NULL)
    AND MAIN_ENTITY_ID_CTXT_TYP = 'EXEC_TRD_ID'
    AND MAIN_ENTITY_TYP = 'EXTR'
    AND MAIN_ENTITY_TBL_TYP = 'EXTR'
    AND CRRNT_SEVERITY_CDE = 0
    AND CRRNT_TRN_STAT_TYP = 'CLOSED'
    AND TRN_MSG_STAT_TYP = 'NOCHANGE'
    AND TRN_MSG_STAT_DESC LIKE '14022019_${PORTFOLIOCRTSID}_FUND_OPEN_END_%_CANC_%'
    AND JOB_ID IN
      (SELECT JOB_ID FROM
