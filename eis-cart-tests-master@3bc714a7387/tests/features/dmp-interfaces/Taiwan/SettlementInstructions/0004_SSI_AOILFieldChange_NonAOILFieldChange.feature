...
@tom_4095 @tom_4095_004_aoilfieldchange_nonaoilfieldchange @tom_4476 @tw_settlement_instruction_format
@eisdev_7439

Feature: Test SSI report for one AOIL field change and one non AOI field change
  ...
  Scenario: Clear table data and setup variables
    ...
    """
     ...
     WHERE TRD_ID IN ('4267-SITEST4') AND END_TMS IS NULL;
     COMMIT
    """
    ...
    WHERE EXEC_TRN_ID IN ('4267-SITEST4')
  ...
  Scenario: Load portfolio, security, broker and trade file
    ...
    And I assign "004_new_trade_CIS.xml" to variable "INPUT_FILENAME3"
  ...
  Scenario: Performing verifications
    ...
    """
      select count(*) TRANS_ROW_COUNT from ft_t_extr where trd_id ='4267-SITEST4' and end_tms is null
    """
    ...
    """
      ...
      WHERE TRD_ID = '4267-SITEST4' AND  END_TMS IS NULL
      )
    """
  ...
  Scenario:  Load Trades file after changing AOIL field - TRD_ORIG_FACE(EXTR.TRD_CQTY) and non AOI field - TRD_SETTLE_LOCATION (STL_LOC_MNEM)

    Given I assign "004_AOILfield_nonAOIfield_change_CIS.xml" to variable "INPUT_FILENAME"
    ...
    And I process "${testdata.path}/testdata/${INPUT_FILENAME}" file with below parameters
    ...
    # Check if TRD_CQTY is changed succesfully
    And I expect value of column "EXTR_TRDPRC_ROW_COUNT" in the below SQL query equals to "1":
    """
      SELECT COUNT(*) AS EXTR_TRDPRC_ROW_COUNT FROM FT_T_EXTR
      WHERE TRD_CQTY = 2000
      AND END_TMS IS NULL
