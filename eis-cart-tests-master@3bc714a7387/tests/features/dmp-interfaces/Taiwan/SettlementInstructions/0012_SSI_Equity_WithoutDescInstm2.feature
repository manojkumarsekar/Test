...
#https://jira.intranet.asia/browse/TOM-4538 - include etf case, filename format changes and allow 3rd gen SI
#https://jira.intranet.asia/browse/TOM-4538 - fix filter query to include ETF
#https://jira.intranet.asia/browse/TOM-4585 - fix filter query to include Equity

@tom_4095 @generateNoETFSI @tom_4585
Feature: Equity settlement instruction report generation (Without DescInstm2)
  This feature file to test ETF settlement instruction report generation
  ...
  Scenario: Clear table data and setup variables
   ...
   ${testdata.path}/sql/InsertAccount.sql
    ...
    """
     ...
     WHERE TRD_ID IN ('4267-SITEST12') AND END_TMS IS NULL;
     COMMIT
    """
    ...
    WHERE EXEC_TRN_ID IN ('4267-SITEST12')
  ...
  Scenario: Load Trades file for testing
    ...
    Given I assign "012_sm_Equity_NoETF.xml" to variable "INPUT_FILENAME1"
    And I assign "broker.xml" to variable "INPUT_FILENAME2"
    And I assign "012_new_trade_CIS.xml" to variable "INPUT_FILENAME3"
  ...
  | 14022019_${PORTFOLIOCRTSID}_EQUITY_EQUITY_CITI MY_BUY_DBSSP_NEWM_*.pdf |
    ...
    And I expect value of column "TRANS_ROW_COUNT" in the below SQL query equals to "1":
      """
      select count(*) TRANS_ROW_COUNT from ft_t_extr where trd_id ='4267-SITEST12' and end_tms is null
      """
 ...
 # Check if ISST is created with DSCINST2 = ETF
    And I expect value of column "ISST_DESCINSMT2_ROW_COUNT" in the below SQL query equals to "0":
      ...
      WHERE TRD_ID = '4267-SITEST12' AND  END_TMS IS NULL
      )
      """
