...
#https://jira.intranet.asia/browse/TOM-4539 - trdcounterparty match ket added

@tom_4095 @SSIonTrdPrincipalChange @tom_4539
Feature: Settlement instruction report generation on 3rd revision
  ...
  Scenario: Clear table data and setup variables
    ...
    And I execute below query
      ...
      WHERE TRD_ID IN ('4267-SITEST14') AND END_TMS IS NULL;
      COMMIT
      """
    ...
    And I execute below query
      ...
      WHERE EXEC_TRN_ID IN ('4267-SITEST14')
  ...
  Scenario: Load Trades file for testing
    ...
    Given I assign "014_sm_Equity_ETF.xml" to variable "INPUT_FILENAME1"
    And I assign "broker.xml" to variable "INPUT_FILENAME2"
    And I assign "014_new_trade_CIS.xml" to variable "INPUT_FILENAME3"
    ...
    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "/dmp/out/taiwan/settlement" if exists:
      | 14022019_${PORTFOLIOCRTSID}_EQUITY_EQUITY_CITI MY_BUY_DBSSP_NEWM_*.pdf |

    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "/dmp/out/taiwan/settlement" if exists:
      | 14022019_${PORTFOLIOCRTSID}_EQUITY_EQUITY_CITI MY_BUY_DBSSP_NEWM_*.pdf.error |
    ...
    And I expect value of column "TRANS_ROW_COUNT" in the below SQL query equals to "1":
      """
      select count(*) TRANS_ROW_COUNT from ft_t_extr where trd_id ='4267-SITEST14' and end_tms is null
      """
    ...
    And I expect value of column "EXST_PROCESSED_ROW_COUNT" in the below SQL query equals to "1":
      ...
      WHERE TRD_ID = '4267-SITEST14' AND  END_TMS IS NULL
      )
      """
  ...
  This scenario is to load the trade file to load data with changed TRDCounterparty to genrate 2nd revision settlement instruction report

    Given I assign "014_trdcpty_changed.xml" to variable "INPUT_FILENAME3"
    And I assign "/dmp/out/taiwan/settlement" to variable "PUBLISHING_DIR"
    ...
    And I expect value of column "TRCP_ROW_COUNT" in the below SQL query equals to "1":
      """
      select count(*) TRCP_ROW_COUNT from ft_t_trcp where EXEC_TRD_ID IN ( SELECT EXEC_TRD_ID FROM FT_T_EXTR
      WHERE TRD_ID = '4267-SITEST14' AND  END_TMS IS NULL
      ) and CNTPRTY_ID = 'DBSS-ES' and FINSRL_TYP is null and end_tms is null
      """
