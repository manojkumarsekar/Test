#https://jira.intranet.asia/browse/TOM-4494
#https://jira.intranet.asia/browse/TOM-4613
#https://jira.intranet.asia/browse/TOM-4652
#https://jira.intranet.asia/browse/TOM-4608
#https://jira.intranet.asia/browse/TOM-4735
#https://jira.intranet.asia/browse/TOM-4773

@dmp_regression_integrationtest
@tom_4494 @tw_settlement_instruction_format @tw_settlement_instruction_format_bond @tom_4608
@tom_4652 @tom_4613 @tom_4735 @tom_4773 @eisdev_7439

Feature: Feature file for SI BOND PDF verification
  ...
  Scenario: Clear table data and setup variables
    Given I assign "tests/test-data/dmp-interfaces/Taiwan/Insight/Bond" to variable "testdata.path"

    And I execute below query
    """
     UPDATE FT_T_EXTR SET END_TMS = SYSDATE
     WHERE TRD_ID IN ('4494-4494') AND END_TMS IS NULL;
     UPDATE FT_T_ETID SET END_TMS = SYSDATE
     WHERE EXEC_TRN_ID IN ('4494-4494') AND EXEC_TRN_ID_CTXT_TYP = 'BRSTRNID' AND END_TMS IS NULL;
     UPDATE FT_T_ACID SET END_TMS = SYSDATE
     WHERE ACCT_ALT_ID = 'U_TT4494';
     COMMIT
    """
  ...
  Scenario: Assigning variables and executing clean up
    ...
    Then I extract value from the xml file "${testdata.path}/testdata/${INPUT_FILENAME4}" with tagName "ISIN" to variable "CUSIP"
    ...
    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${PUBLISHING_DIR}" if exists:
      | 20022019_${PORTFOLIOCRTSID}_BND_CORP_*_COLL_CPC_NEWM_*.pdf |

    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${PUBLISHING_DIR}" if exists:
      | 20022019_${PORTFOLIOCRTSID}_BND_CORP_*_COLL_CPC_NEWM_*.pdf.error |
  ...
  Scenario: Performing portfolio verification
    ...
    """
      select count(*) PORTFOLIO_ROW_COUNT from ft_t_acid where acct_alt_id ='U_TT4494' and end_tms is null
    """
  ...
  Scenario: Performing broker verification
    ...
    """
      select count(*) BROKER_ROW_COUNT from ft_t_FIID where fins_id ='TEST_4494' and end_tms is null
    """
  ...
  Scenario: Performing transaction verification
    ...
    """
      select count(*) TRANS_ROW_COUNT from ft_t_extr where trd_id ='4494-4494' and end_tms is null
    """
  ...
  Scenario: Run publish document workflow for trade
    ...
    """
      ...
      WHERE TRD_ID = '4494-4494' AND  END_TMS IS NULL
      )
    """
  ...
  Scenario: Loading PDF file in local and performing direct PDF comparison with expected PDF
    ...
    Given I assign "20022019_U_TT4494_BOND_BASE.pdf" to variable "EXPECTED_FILE"

    When I read latest file with the pattern "20022019_${PORTFOLIOCRTSID}_BND_CORP_*_COLL_CPC_NEWM_*.pdf" in the path "${PUBLISHING_DIR}" with the host "dmp.ssh.inbound" into variable "LATEST_FILE_NAME"
