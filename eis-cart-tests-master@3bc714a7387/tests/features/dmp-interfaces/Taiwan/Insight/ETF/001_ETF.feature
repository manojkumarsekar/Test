#https://jira.intranet.asia/browse/TOM-4465
...
@tom_4465 @tw_settlement_instruction_format @tw_settlement_instruction_format_ETF
@tom_4608 @tom_4652 @tom_4613 @eisdev_7439

Feature: Feature file for SI ETF PDF verification
  ...
  Scenario: Clear table data and setup variables
    Given I assign "tests/test-data/dmp-interfaces/Taiwan/Insight/ETF" to variable "testdata.path"

    And I execute below query
    """
     UPDATE FT_T_EXTR SET END_TMS = SYSDATE
     WHERE TRD_ID IN ('14882-33') AND END_TMS IS NULL;
	 UPDATE FT_T_ETID SET END_TMS = SYSDATE
     WHERE EXEC_TRN_ID IN ('14882-33') AND EXEC_TRN_ID_CTXT_TYP = 'BRSTRNID' AND END_TMS IS NULL;
     UPDATE FT_T_ACID SET END_TMS = SYSDATE
     WHERE ACCT_ALT_ID = 'U_TT4465';
     COMMIT
    """
  ...
  Scenario: Assigning variables and executing clean up
    ...
    #Then I extract value from the xml file "${testdata.path}/testdata/${INPUT_FILENAME2}" with tagName "ISIN" to variable "ISIN"
    Then I extract value from the xml file "${testdata.path}/testdata/${INPUT_FILENAME2}" with xpath "//CUSIP_ALIAS_set//CODE[text()='70']/../IDENTIFIER" to variable "ISIN"
    ...
    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${PUBLISHING_DIR}" if exists:
      | 13022019_${PORTFOLIOCRTSID}_EQUITY_EQUITY_*_NEWM_*.pdf |

    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${PUBLISHING_DIR}" if exists:
      | 13022019_${PORTFOLIOCRTSID}_EQUITY_EQUITY_*_NEWM_*.pdf.error |
  ...
  Scenario: Performing portfolio verification
    ...
    """
      select count(*) PORTFOLIO_ROW_COUNT from ft_t_acid where acct_alt_id ='U_TT4465' and end_tms is null
    """
  ...
  Scenario: Performing Security verification
    ...
    """
      select count(*) SEC_ROW_COUNT from ft_t_isid where iss_id ='SBKM4GZ66' and end_tms is null
    """
  ...
  Scenario: Performing broker verification
    ...
    """
      select count(*) BROKER_ROW_COUNT from ft_t_FIID where fins_id ='TEST_4465' and end_tms is null
    """
  ...
  Scenario: Performing transaction verification
    ...
    """
    select count(*) TRANS_ROW_COUNT from ft_t_extr where trd_id ='14882-33' and end_tms is null
    """
  ...
  Scenario: Run publish document workflow for trade
    ...
    """
      ...
      WHERE TRD_ID = '14882-33' AND  END_TMS IS NULL
      )
    """
  ...
  Scenario: Loading PDF file in local and performing direct PDF comparison with expected PDF
    ...
    Given I assign "14882-33_ETF_BASE.pdf" to variable "EXPECTED_FILE"

    When I read latest file with the pattern "13022019_${PORTFOLIOCRTSID}_EQUITY_EQUITY_*_NEWM_*.pdf" in the path "${PUBLISHING_DIR}" with the host "dmp.ssh.inbound" into variable "LATEST_FILE_NAME"
