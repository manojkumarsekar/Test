...
Feature: Test creation of shell security from research report  (order file -> report email -> BRS)
  ...
  This feature is about Load order file followed by research report email info converted to xml file through interface and BRS security file
  ...
  Scenario: TC2: Loading order file in DMP
 ...
 #Mail verification to be done manually
  ...
  Scenario: TC3: Verification of Security loaded from Order file in FT_T_ISID table
  ...
  Scenario: TC4: Verification of Security mapping loaded from Order file in FT_T_ISID table for dummy MKT_OID
  ...
  Scenario: TC5: Verification of Security mapping loaded from Order file in FT_T_MKIS table for dummy MKT_OID
  ...
  Scenario: TC7: Verification of Security loaded from Reasearch Report in FT_T_ISID table

    Then I expect value of column "ISID_ROW_COUNT" in the below SQL query equals to "4":
      """
      SELECT count(*) as ISID_ROW_COUNT FROM FT_T_ISID WHERE
      INSTR_ID in
      (
  ...
  Scenario: TC8: Verification of Security mapping loaded from Research Report in FT_T_ISID table for dummy MKT_OID

    Then I expect value of column "ISID_ROW_COUNT_MKTOID" in the below SQL query equals to "3":
      """
      SELECT count(*) as ISID_ROW_COUNT_MKTOID FROM FT_T_ISID WHERE
      INSTR_ID =
        (
  ...
  Scenario: TC9: Verification of Security mapping loaded from Reasearch Report in FT_T_MKIS table for dummy MKT_OID
