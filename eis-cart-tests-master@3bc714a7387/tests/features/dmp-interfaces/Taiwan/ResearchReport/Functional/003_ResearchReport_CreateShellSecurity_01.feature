...
@tom_4236 @dmp_twrr_functional @dmp_tw_functional
Feature: Test creation of shell security from research report  (report email -> order file -> BRS)
  ...
  This feature is about Load research report email info converted to xml file through interface followed by order file and BRS security file
  ...
  Scenario: TC2: Loading Research report email in DMP
  ...
  Scenario: TC3: Verification of Security loaded from Reasearch Report in FT_T_ISID table

    Then I expect value of column "ISID_ROW_COUNT" in the below SQL query equals to "3":
      """
  ...
  Scenario: TC4: Verification of Security mapping loaded from Research Report in FT_T_ISID table for dummy MKT_OID

    Then I expect value of column "ISID_ROW_COUNT_MKTOID" in the below SQL query equals to "2":
      """
      ...
      INSTR_ID =
       ...
       AND MKT_OID IN
        (
          select MKT_OID from ft_T_mkid where mkt_id = 'ZZZZ' and mkt_id_ctxt_typ = 'MIC'
        )
      AND END_TMS IS NULL
      """

  Scenario: TC5: Verification of Security mapping loaded from Reasearch Report in FT_T_MKIS table for dummy MKT_OID

    Then I expect value of column "MKIS_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS MKIS_COUNT FROM FT_T_MKIS
      WHERE INSTR_ID =
        (
          SELECT INSTR_ID FROM FT_T_ISID WHERE ISS_ID = '${BCUSIP}' AND END_TMS IS NULL
        )
  ...
  Scenario: TC6 : Verification of security loaded from Reasearch report in FT_T_RSR1 table
  ...
  Scenario: TC7: Loading order file in DMP
  ...
  Scenario: TC8: Verification of Security loaded from Order file in FT_T_ISID table

    And I expect value of column "ISID_ROW_COUNT" in the below SQL query equals to "4":
      """
      SELECT count(*) as ISID_ROW_COUNT FROM FT_T_ISID WHERE
      INSTR_ID in
      (
  ...
  Scenario: TC9: Verification of Security mapping loaded from Order file in FT_T_ISID table for dummy MKT_OID

    And I expect value of column "ISID_ROW_COUNT_MKTOID" in the below SQL query equals to "3":
    """
    SELECT count(*) as ISID_ROW_COUNT_MKTOID FROM FT_T_ISID WHERE
    INSTR_ID =
    (
  ...
  Scenario: TC10: Verification of Security mapping loaded from Order file in FT_T_MKIS table for dummy MKT_OID
  ...
  Scenario: TC11: Loading BRS security file in DMP
