...
@dmp_taiwan
@tom_3844 @tom_3385 @tw_bondgovt_trades
Feature: Inbound Trades Interface Testing (R5.IN-TRAN10 Trades BRS to DMP) -Verify Bond GOVT loaded in DMP
  ...
  Scenario: TC1: Clear table data and setup variables
    ...
    """
     ...
     WHERE TRD_ID IN ('12627-73') AND END_TMS IS NULL;
     COMMIT
    """
    ...
    WHERE EXEC_TRN_ID IN ('12627-73') AND EXEC_TRN_ID_CTXT_TYP = 'BRSTRNID' AND END_TMS IS NULL;
    COMMIT
    """
  ...
  Scenario: TC2: Load Trades file for new Bond Govt security transaction in portfolio TT56 (INVNUM=-73)
  ...
  2) Verify a new record created for this transaction and all the required fields are mapped properly in DMP
    ...
    Given I assign "BondGovt_transaction_New_Tem.xml" to variable "INPUT_TEMPLATE_FILENAME"
    And I assign "BondGovt_transaction_New.xml" to variable "INPUT_FILENAME"
   ...
   # Check if EXTR is created with check TRD_ID, Tran_Type1, TradeDate, Settledate, Trd_CCy, Trd_Price, Trd_Org_Face from test data
    And I expect value of column "EXTR_PROCESSED_ROW_COUNT" in the below SQL query equals to "1":
      ...
      FROM FT_T_EXTR
      WHERE TRD_ID = '12627-73' AND   EXEC_TRN_CAT_TYP = 'TRD' AND TO_CHAR (TRD_DTE, 'MM/DD/YYYY') = '11/28/2018'
      AND TO_CHAR (SETTLE_DTE, 'MM/DD/YYYY') = '11/30/2018'
      AND TRD_CURR_CDE = 'TWD' AND TRD_CPRC = '100.0000000000' AND  TRN_CDE = 'BRSEOD' AND TRD_CQTY = '-1000.0000000000' AND END_TMS IS NULL
      """
    ...
    And I expect value of column "ETAM_PROCESSED_ROW_COUNT" in the below SQL query equals to "1":
      ...
      WHERE EXTN_CURR_CDE = 'TWD' AND NET_SETTLE_CAMT = '-1000.0000000000'
      AND TRADED_IN_CAMT = '-5.0000000000' AND ACRD_IN_CAMT = '1.3000000000' AND EXEC_TRD_ID IN ( SELECT EXEC_TRD_ID FROM FT_T_EXTR
      WHERE TRD_ID = '12627-73' AND END_TMS IS NULL
      )
      """
    ...
    And I expect value of column "ETCM_PROCESSED_ROW_COUNT" in the below SQL query equals to "4":
      ...
      WHERE CMNT_REAS_TYP IN ('TRADER', 'TRDMODBY', 'TRDREVIEW', 'TRDCOMMENTS')
      AND EXEC_TRD_ID IN ( SELECT EXEC_TRD_ID FROM FT_T_EXTR
      WHERE TRD_ID = '12627-73' AND END_TMS IS NULL
      )
      """
    ...
    And I expect value of column "EXMB_PROCESSED_ROW_COUNT" in the below SQL query equals to "1":
      ...
      AND CRRNT_FACE_CAMT = '-1005.0000000000'
      AND POOL_FACTOR_CRTE = '1.0000000000'
      AND ORIG_FACE_CAMT = '-1000.0000000000'
      AND EXEC_TRD_ID IN ( SELECT EXEC_TRD_ID FROM FT_T_EXTR
      WHERE TRD_ID = '12627-73' AND  END_TMS IS NULL
      )
      """
    ...
    And I expect value of column "EXFI_PROCESSED_ROW_COUNT" in the below SQL query equals to "1":
      ...
      WHERE WORST_YIELD_CPCT = '1.1222939900' AND CALL_OPT_YIELD_CPCT = '1.2000000000' AND DIRTY_CPRC = '1.3' AND EXEC_TRD_ID IN ( SELECT EXEC_TRD_ID FROM FT_T_EXTR
      WHERE TRD_ID = '12627-73' AND END_TMS IS NULL
      )
      """

  Scenario: TC3: Load amended Trades file for Bond Govt security transaction in portfolio TT56 (INVNUM=-73)
  Expected Result: 1) File should load and make entry in jblg
  2) Verify old record updated for this transaction instead of creating the new record in DMP

    Given I assign "BondGovt_transaction_Amend_Tem.xml" to variable "INPUT_TEMPLATE_FILENAME"
    And I assign "BondGovt_transaction_Amend.xml" to variable "INPUT_FILENAME"
    ...
    And I expect value of column "EXTR_PROCESSED_ROW_COUNT" in the below SQL query equals to "1":
      ...
      WHERE TRD_ID = '12627-73' AND   EXEC_TRN_CAT_TYP = 'TRD' AND TO_CHAR (TRD_DTE, 'MM/DD/YYYY') = '11/28/2018'
      AND TO_CHAR (SETTLE_DTE, 'MM/DD/YYYY') = '11/30/2018'
      AND TRD_CURR_CDE = 'TWD' AND TRD_CPRC = '200.0000000000' AND  TRN_CDE = 'BRSEOD' AND TRD_CQTY = '-5000.0000000000' AND END_TMS IS NULL
      """
    ...
    And I expect value of column "EXMB_PROCESSED_ROW_COUNT" in the below SQL query equals to "1":
      ...
      AND CRRNT_FACE_CAMT = '-1005.0000000000'
      AND POOL_FACTOR_CRTE = '1.0000000000'
      AND ORIG_FACE_CAMT = '-5000.0000000000'
      AND EXEC_TRD_ID IN ( SELECT EXEC_TRD_ID FROM FT_T_EXTR
      WHERE TRD_ID = '12627-73' AND END_TMS IS NULL
      )
      """

  Scenario: TC4: Load cancelled Trades file for Bond Govt security transaction in portfolio TT56 (INVNUM=-73)
  Expected Result: 1) File should load and make entry in jblg
  2) Verify old record cancelled for this transaction instead of creating the new record in DMP

    Given I assign "BondGovt_transaction_Cancel_Tem.xml" to variable "INPUT_TEMPLATE_FILENAME"
    And I assign "BondGovt_transaction_Cancel.xml" to variable "INPUT_FILENAME"
