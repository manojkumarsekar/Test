...
Feature: This feature tests the translations performed by DMP on SSB's non-FX position file prior to forwarding to BRS
  ...
  Scenario: TC1: Setup test data
    ...
    And I assign "ESI_BRS_POSITION_NONFX_SSB.csv" to variable "INPUT_FILENAME"
    And I assign "ESI_BRS_POSITION_SSB_NONFX_B_${VAR_SYSDATE}.csv" to variable "OUTPUT_FILENAME"

  Scenario: TC2: Run SSB non-FX position file transformation
    ...
    And I remove below files in the host "dmp.ssh.outbound" from folder "${dmp.ssh.outbound.path}/brs" if exists:
      | ESI_BRS_POSITION_SSB_NONFX_B*.csv |

    And I set the workflow template parameter "FILE" to "${dmp.ssh.inbound.path}/ssb/${INPUT_FILENAME}"
    And I set the workflow template parameter "MESSAGE_TYPE" to "EITW_MT_SSB_NONFX_POSITION_TO_BRS"
