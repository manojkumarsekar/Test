...
@dmp_taiwan
@tom_4422 @tw_fx_shareclass_mainportfolio_file367 @tom_4825 @tw_fx_trade_file367 @tom_4826
Feature: Load BRS FX SPOT transaction for hedge portfolio and generate file 367( New Cash)
  ...
  Scenario Outline: validate File 367(New cash file for BRS) generated from FX SPOT trade <Scenario Name>
    ...
    Examples: Trade Parameters
      ...
      | Load buy fx/spot(AUD/USD) trade without confirmedBy and check new cash | 10000           | AUD       | USD         | SPOT       |                | 1             |           | ${SPLIT_PORTFOLIO_NAME} | ${SPLIT_PORTFOLIO_NAME} | CASHIN       | CASHOUT      | 20190528     | E              |
      | Load SELL fx/spot(AUD/USD) trade with confirmedBy and check new cash   | -10000          | AUD       | USD         | SPOT       | JIL            | 1             |           | ${SPLIT_PORTFOLIO_NAME} | ${SPLIT_PORTFOLIO_NAME} | CASHOUT      | CASHIN       | 20190528     | F              |
      | Load buy fx/spot(TWD/USD) trade with confirmedBy and check new cash    | 10000           | TWD       | USD         | SPOT       | JIL            | 1             |           | ${SPLIT_PORTFOLIO_NAME} | ${MAIN_PORTFOLIO_NAME}  | CASHIN       | CASHOUT      | 20190528     | F              |
      | Load Sell fx/spot(AUD/TWD) trade with confirmedBy and check new cash   | -10000          | AUD       | TWD         | SPOT       | JIL            | 1             |           | ${MAIN_PORTFOLIO_NAME}  | ${SPLIT_PORTFOLIO_NAME} | CASHOUT      | CASHIN       | 20190528     | F              |
