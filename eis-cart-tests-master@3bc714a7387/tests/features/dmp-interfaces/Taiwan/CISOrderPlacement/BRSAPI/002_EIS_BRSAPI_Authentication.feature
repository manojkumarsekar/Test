@tom_3700 @002_brs_api_invalid_credentials @cis_op_regression @cis_op_brsapi

Feature: CISOrderPlacement | BRS API | F002 |Test BRS authentication in Publish Document workflow
  Test the Publish document workflow with wrong BRS credentials
  ...
  Scenario: TC_1: Run publish document with wrong BRS credentials
  ...
  #Verify Order data
    ...
    When I process publish document workflow with below parameters and wait for the job to be completed
      ...
      | BRSPROPERTY_FILE_LOCATION      | ${brscredentials.invalidvalidfilelocation} |
    ...
    And I expect value of column "NTEL_RECORD_COUNT" in the below SQL query equals to "1":
      ...
      AND PARM_VAL_TXT like '%not process the request as the necessary credentials were not provided or were invalid%'
      """
