@tom_3700 @003_brs_missing_details @cis_op_regression @cis_op_brsapi

Feature: CISOrderPlacement | BRS API | F003 |Test Publish Document workflow with missing BRS API and Credentials
  Test the Publish document workflow with missing BRS details BRS_WEBSERVICE_URL and BRSPROPERTY_FILE_LOCATION
  ...
  Scenario: TC_1: Run publish document with missing BRS parameters
  Expected Result: Workflow should fail  Instance ID will be generated and job will keep on running in started status in WFRI table
  ...
  #Verify Order data
    ...
    And I set the workflow template parameter "BRS_WEBSERVICE_URL" to ""
    And I set the workflow template parameter "BRSPROPERTY_FILE_LOCATION" to ""
