...
@tom_4612 @tom_4677 @tw_order_placement @cis_op_regression @cis_op_functional
@eisdev_7439

Feature: CISOrderPlacement | Functional | F014_4 | CIS Order Placement Format 2 for split portfolio changes

  1. Load new order
  2. Perform verification of data inserted
  3. Publish PDF document by calling workflow
  ...
  Scenario: Clear data and assign variables
    ...
    LAST_CHG_TMS = SYSDATE WHERE PREF_ORDER_ID = '2426892_TEST' AND PREF_ORDER_ID_CTXT_TYP = 'BRS_ORDER';
    COMMIT
    """
    ...
    And I assign "013_esi_orders.20190514_0445_format2.xml" to variable "INPUT_FILENAME"

    When I extract value from the xml file "${TESTDATA_PATH}/order/testdata/${INPUT_FILENAME}" with tagName "CUSIP" to variable "BCUSIP"
    When I extract value from the xml file "${TESTDATA_PATH}/order/testdata/${INPUT_FILENAME}" with tagName "PORTFOLIO_NAME" to variable "PORTFOLIOCRTSID"
    ...
    Given I execute below query
    """
	${TESTDATA_PATH}order/sql/DELETE_ESUNPLTF_FOR_PORTFOLIO_FORMAT2.sql
    """
  ...
  Scenario: Load orders for portfolio.
    ...
    AND AUOR.PREF_ORDER_ID IN ('2426892_TEST')
  ...
  Scenario: Test BRS API Call & Publish document by doing a API call to Insight
    ...
    Then I expect below files with pattern are not available in the host "dmp.ssh.inbound" from folder "${PUBLISHING_DIRECTORY}" after processing:
      | 2426892_TEST_${PORTFOLIOCRTSID}_${BCUSIP}_*_ACTIVE_*.pdf.error |

  Scenario: Loading PDF file in local and performing direct PDF comparison with expected PDF

    Given I assign "ORDER_PLACEMENT_FORMAT2_BASE.pdf" to variable "EXPECTED_FILE"
    When I read latest file with the pattern "2426892_TEST_${PORTFOLIOCRTSID}_${BCUSIP}_*_ACTIVE_*.pdf" in the path "${PUBLISHING_DIRECTORY}" with the host "dmp.ssh.inbound" into variable "LATEST_FILE_NAME"
