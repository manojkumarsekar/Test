@tom_3700 @tom_3700_cancelorder @cis_op_regression @cis_op_functional

Feature: CISOrderPlacement | Functional | F006 | Test Cancel Order in Publish Document workflow
  This is to test the cancel order
  ...
  #Assign Variables
    ...
    And I assign "006_esi_activeorder_publish_document.xml" to variable "INPUT_FILENAME"
 ...
 #Pre-requisite : Clear Orders
    ...
    Given I execute below query
	"""
   ${TESTDATA_PATH}order/sql/INSERT_Y_ESUNPLTF_TT107.sql
  """
  ...
  #Pre-requisite : Clear BRS attributes
    ...
    And I set the workflow template parameter "EMAIL_SUBJECT" to "TEST CANCEL ORDER"
  ...
  Scenario: Load cancel order for E-SUN portfolio.

  #Assign Variables
    Given I assign "/dmp/out/taiwan/placement" to variable "PUBLISHING_DIRECTORY"
    And I assign "tests/test-data/dmp-interfaces/Taiwan/CISOrderPlacement/" to variable "TESTDATA_PATH"
    And I assign "006_esi_cancelorder_publish_document.xml" to variable "INPUT_FILENAME"
  ...
  #Load Data
    ...
    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${PUBLISHING_DIRECTORY}" if exists:
      | ${ORDNUM}_${PORTFOLIOCRTSID}_${BCUSIP}_*_CANCELLED_*.pdf |

    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${PUBLISHING_DIRECTORY}" if exists:
      | ${ORDNUM}_${PORTFOLIOCRTSID}_${BCUSIP}_*_CANCELLED_*.pdf.error |
  ...
  Scenario: Run publish document workflow for active order
   ...
   #Verify if Insight Call transaction is closed succesfully
    ...
    #Verify Order status
    Then I expect value of column "INSIGHTCANCORDSTATCNT" in the below SQL query equals to "1":
    """
   ${TESTDATA_PATH}order/sql/VERIFY_INSIGHTCANCORDERSTATUS.sql
    """
