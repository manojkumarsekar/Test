@tom_3700 @tom_3700_nochangeorder @cis_op_regression @cis_op_functional

Feature: CISOrderPlacement | Functional | F009 | Test no change Order in Publish Document workflow
  This is to test no change order
  ...
  #Assign Variables
    ...
    And I assign "009_esi_activeorder_publish_document.xml" to variable "INPUT_FILENAME"
  ...
  #Pre-requisite : Clear BRS attributes
    ...
    And I set the workflow template parameter "EMAIL_SUBJECT" to "TEST NO CHANGE ORDER"
  ...
  Scenario: Load order again for E-SUN portfolio.

  #Assign Variables
    Given I assign "/dmp/out/taiwan/placement" to variable "PUBLISHING_DIRECTORY"
    And I assign "tests/test-data/dmp-interfaces/Taiwan/CISOrderPlacement/" to variable "TESTDATA_PATH"
    And I assign "009_esi_activeorderagain_publish_document.xml" to variable "INPUT_FILENAME"
  ...
  Scenario: Run publish document workflow
  ...
  #Verify if Insight Call transaction is closed succesfully
    Then I expect value of column "NOCHGORDERTRIDCOUNT" in the below SQL query equals to "1":
    """
    SELECT COUNT(*) AS NOCHGORDERTRIDCOUNT FROM FT_T_TRID
    WHERE MAIN_ENTITY_ID = (SELECT AUOR_OID FROM FT_T_AUOR WHERE PREF_ORDER_ID = '${ORDNUM}' AND PREF_ORDER_ID_CTXT_TYP = 'BRS_ORDER' AND ACCT_ID IS NOT NULL )
    ...
    AND TRN_MSG_STAT_DESC LIKE '${ORDNUM}_${PORTFOLIOCRTSID}_${BCUSIP}_%_ACTIVE_%'
    AND JOB_ID IN
      (SELECT JOB_ID FROM
