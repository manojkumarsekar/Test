...
@dmp_taiwan
@tom_4818 @trade_recap @trade_recap_hsbc_b1 @eisdev_5941 @eisdev_7114
Feature: Test Publishing of HSBC Trade Recap data for batch 1 & trade count report
  ...
  Scenario: Load Fresh data for Trades 4818-4818
    ...
    And I execute below query to "Clear old test data and setup variables"
    """
  ...
  Scenario: Setup new account group and Financial Role Account Participant data in DMP linked to above account
    ...
    """
    ${testdata.path}/sql/SetUp_ACGR_CAP1_FRAP_HSBC_4818.sql
    """
  ...
  Scenario: Publish trade recap file for HSBC
    ...
    Given I assign "traderecap_hsbc_out_file_1_B1_4818" to variable "PUBLISHING_FILE_NAME"
...
#Check if PORTFOLIO CUSIP has value BPM21EWK5 in the outbound
  ...
  Scenario: Remove operational data for trade recap publishing
    ...
    AND    subscription_nme LIKE 'EITW%HSBC%TRADEFLOW%B1%'
    """

  Scenario: Publish trade recap file for HSBC

    Given I assign "traderecap_hsbc_out_file_2_B1_4818" to variable "PUBLISHING_FILE_NAME"
    And I assign "/dmp/out/hsbc" to variable "publishDirectory"
    And I generate value with date format "YYYYMMdd" and assign to variable "VAR_SYSDATE"
    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${publishDirectory}" if exists:
      | ${PUBLISHING_FILE_NAME}_*.csv |
...
#Check if PORTFOLIO CUSIP has value BPM21EWK5 in the outbound
  ...
  Scenario: Run trade recap record count control report
    ...
    Given I assign "esi_TW_TRConReport_HSBC_${VAR_SYSDATE}.htm" to variable "OUTPUT_FILENAME"
    ...
    And I process the workflow template file "tests/test-data/intf-specs/gswf/template/EIS_XmlQueryTransform/request.xmlt" with below parameters and wait for the job to be completed
      ...
      | EMAIL_SUBJECT    | Trade recap feature file run (HSBC)                                 |
      | FILENAME         | /dmp/out/taiwan/${OUTPUT_FILENAME}                                  |
      | QUERY_PARAMETER1 | FundAdmin:HSBC                                                      |
      | QUERY_PARAMETER2 | Batch:B1                                                            |
    ...
    Then I expect value from xml file "${testdata.path}/outfiles/runtime/${OUTPUT_FILENAME}" with xpath "/html/body/table/tr/th[3]" should be "FUND/OPEN_END"

    And I expect value from xml file "${testdata.path}/outfiles/runtime/${OUTPUT_FILENAME}" with xpath "/html/body/table/tr/th[4]" should be "OPTION/CUROTC"
    ...
    And I expect value from xml file "${testdata.path}/outfiles/runtime/${OUTPUT_FILENAME}" with xpath "/html/body/table/tr[2]/td[4]" should be "1"

    #Validate Total Count EXECSENT + SENT
    And I expect value from xml file "${testdata.path}/outfiles/runtime/${OUTPUT_FILENAME}" with xpath "/html/body/table/tr[td[1]='Total']/td[5]" should be "2"
