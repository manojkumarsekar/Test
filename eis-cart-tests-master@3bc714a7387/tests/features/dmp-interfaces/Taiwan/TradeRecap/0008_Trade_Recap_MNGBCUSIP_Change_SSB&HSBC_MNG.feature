...
@dmp_taiwan @tom_4815 @trade_recap  @tom_5364 @tom_5072 @eisdev_4567 @eisdev_6990 @eisdev_6990_ssb_hsbc_mng
Feature: This feature file is to test Trade Recap data published with MNGBCUSIP identifier for SSB & HSBC (MNG).
  ...
  This feature file to test above scenarios with SSB & HSBC - MnG - trade recap publishing
  ...
  Scenario: Clear any residual prod copy trades recaps by running the report once for SSB
  ...
  Scenario: Load Fresh data for Trades
    ...
    Given I assign "sm_MNG_TOM_4815.xml" to variable "INPUT_FILENAME_SM"
    And I assign "CounterParty_TOM_4815.xml" to variable "INPUT_FILENAME_CP"
    And I assign "tradefile_ssb_MNG_TOM_4815.xml" to variable "INPUT_FILENAME_TXN"

    Then I process "${testdata.path}/infiles/${INPUT_FILENAME_SM}" file with below parameters
      | BUSINESS_FEED |                         |
    ...
    And I process "${testdata.path}/infiles/${INPUT_FILENAME_CP}" file with below parameters
      | BUSINESS_FEED |                         |
      | FILE_PATTERN  | ${INPUT_FILENAME_CP}    |
      | MESSAGE_TYPE  | EIS_MT_BRS_COUNTERPARTY |

    Then I expect workflow is processed in DMP with total record count as "1"

    And I process "${testdata.path}/infiles/${INPUT_FILENAME_TXN}" file with below parameters
      | BUSINESS_FEED |                                     |
  ...
  Scenario Outline: Publish trade recap file for <Entity> - MnG
    ...
    Examples:
      ...
      | SSB    | traderecap_ssb_MnG  | /dmp/out/ssb  | EITW_DMP_TO_SSB_TRADEFLOW_B1  | PUBLISHING_FILE_NAME_SSB  |
      | HSBC   | traderecap_hsbc_MnG | /dmp/out/hsbc | EITW_DMP_TO_HSBC_TRADEFLOW_B1 | PUBLISHING_FILE_NAME_HSBC |

  Scenario: Verify trade recap SSB file - MnG
    Given I assign "TradeRecapExpected_ssb_MnG_tom_4815_B1_template.csv" to variable "EXPECTED_FILE_TEMPLATE"
    And I assign "TradeRecapExpected_ssb_MnG_tom_4815_B1.csv" to variable "EXPECTED_FILE"
  ...
  Scenario: Verify trade recap HSBC file - MnG

    Given I assign "TradeRecapExpected_hsbc_MnG_tom_4815_B1_template.csv" to variable "EXPECTED_FILE_TEMPLATE"
    And I assign "TradeRecapExpected_hsbc_MnG_tom_4815_B1.csv" to variable "EXPECTED_FILE"
    And I create input file "${EXPECTED_FILE}" using template "${EXPECTED_FILE_TEMPLATE}" from location "${testdata.path}/outfiles"
