...
Feature: This feature file is to test Trade Recap CP/REP (money market) data published with Batch 1.
  ...
  Scenario: Publish trade recap file for HSBC
    ...
    Given I assign "traderecap_hsbc_CP_REPO_B1_01" to variable "PUBLISHING_FILE_NAME"
    And I assign "/dmp/out/hsbc" to variable "publishDirectory"
    And I generate value with date format "YYYYMMdd" and assign to variable "VAR_SYSDATE"
    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${publishDirectory}" if exists:
      | ${PUBLISHING_FILE_NAME}_*.csv |
  ...
  Scenario: Load confirmed trade to verify if it is getting published after Executed trade
    ...
    And I execute below query to "Updates of COUP_FREQ to check else condition of day_ and month_"
    """
  ...
  Scenario: Publish trade recap file for HSBC
    ...
    Given I assign "traderecap_hsbc_CP_REPO_B1_02" to variable "PUBLISHING_FILE_NAME"
    And I assign "/dmp/out/hsbc" to variable "publishDirectory"
    And I generate value with date format "YYYYMMdd" and assign to variable "VAR_SYSDATE"
    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${publishDirectory}" if exists:
      | ${PUBLISHING_FILE_NAME}_*.csv |
