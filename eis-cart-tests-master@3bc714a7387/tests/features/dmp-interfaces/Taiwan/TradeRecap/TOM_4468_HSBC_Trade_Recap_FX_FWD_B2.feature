...
@dmp_taiwan @tom_4468 @trade_recap @tom_4468_B2 @eisdev_6321
Feature: This feature file is to test Trade Recap FX Forward data published with Batch 2.
  ...
  Scenario: Clear any residual prod copy trades recaps by running the report once
    ...
    Given I assign "traderecap_hsbc_FX_FWD_B2" to variable "PUBLISHING_FILE_NAME"
    ...
    When I process publishing wrapper with below parameters and wait for the job to be completed
      ...
      | SUBSCRIPTION_NAME           | EITW_DMP_TO_HSBC_TRADEFLOW_FX_FWD_B2_SUB |
  ...
  Scenario: Setup new account group and Financial Role Account Participant data in DMP linked to above account
    ...
    And I execute below query
    """
    ${testdata.path}/sql/SetUp_FRAP_HSBC_FX_FWD_B2_TOM_4468.sql
    """
  ...
  Scenario: Publish trade recap file for HSBC
    ...
    Given I assign "traderecap_hsbc_FX_FWD_B2_01" to variable "PUBLISHING_FILE_NAME"
    And I assign "/dmp/out/hsbc" to variable "publishDirectory"
    And I generate value with date format "YYYYMMdd" and assign to variable "VAR_SYSDATE"
    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${publishDirectory}" if exists:
      | ${PUBLISHING_FILE_NAME}_*.csv |
  ...
  Scenario: Verify trade recap HSBC file
    ...
    And I create input file "TradeRecapExpected_hsbc_tom_4468_FX_FWD_B2_01.csv" using template "TradeRecapExpected_hsbc_tom_4468_FX_FWD_B2_01_template.csv" with below codes from location "${testdata.path}/outfiles"
      |  |  |

    Then I expect each record in file "${testdata.path}/outfiles/runtime/${PUBLISHING_FILE_NAME}_${VAR_SYSDATE}_1.csv" should exist in file "${testdata.path}/outfiles/testdata/TradeRecapExpected_hsbc_tom_4468_FX_FWD_B2_01.csv" and exceptions to be written to "${testdata.path}/outfiles/runtime/${PUBLISHING_FILE_NAME}_${VAR_SYSDATE}TradeRecapExpected_hsbc_tom_4468_FX_FWD_B2_01.csv" file
  ...
  Scenario: Publish trade recap file for HSBC
    ...
    Given I assign "traderecap_hsbc_FX_FWD_B2_02" to variable "PUBLISHING_FILE_NAME"
    And I assign "/dmp/out/hsbc" to variable "publishDirectory"
    And I generate value with date format "YYYYMMdd" and assign to variable "VAR_SYSDATE"
    And I remove below files with pattern in the host "dmp.ssh.inbound" from folder "${publishDirectory}" if exists:
      | ${PUBLISHING_FILE_NAME}_*.csv |
  ...
  Scenario: Verify trade recap HSBC file
    ...
    And I modify date "${VAR_SYSDATE}" with "-0d" from source format "YYYYMMdd" to destination format "YYYYMMdd" and assign to "CURR_DATE"
    And I create input file "TradeRecapExpected_hsbc_tom_4468_FX_FWD_B2_02.csv" using template "TradeRecapExpected_hsbc_tom_4468_FX_FWD_B2_02_template.csv" with below codes from location "${testdata.path}/outfiles"
      |  |  |

    Then I expect each record in file "${testdata.path}/outfiles/runtime/${PUBLISHING_FILE_NAME}_${VAR_SYSDATE}_1.csv" should exist in file "${testdata.path}/outfiles/testdata/TradeRecapExpected_hsbc_tom_4468_FX_FWD_B2_02.csv" and exceptions to be written to "${testdata.path}/outfiles/runtime/${PUBLISHING_FILE_NAME}_${VAR_SYSDATE}TradeRecapExpected_hsbc_tom_4468_FX_FWD_B2_02.csv" file