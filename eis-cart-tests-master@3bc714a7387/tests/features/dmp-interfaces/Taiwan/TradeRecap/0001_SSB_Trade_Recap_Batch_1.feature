...
@dmp_taiwan
@dmp_gs_upgrade
@tom_3383 @trade_recap @tom_4664 @trade_recap_ssb_b1 @tom_4694 @tom_4783 @eisdev_4693 @eisdev_6321
Feature: Test Publish Trade Recap To SSB for batch 1
  ...
  Scenario: Setup new account group and Financial Role Account Participant data in DMP linked to above account
    ...
    And I execute below query
    """
    ${testdata.path}/sql/SetUp_ACGR_CAP1_FRAP_SSB.sql
    """
  ...
  Scenario: Publish trade recap file for SSB (2 Trade records TRD_ID ='3204-302' and '3204-2776_valid_trade_parent)
    ...
    Given I assign "traderecap_ssb_out_file_b1_01" to variable "PUBLISHING_FILE_NAME"
    And I assign "/dmp/out/ssb" to variable "publishDirectory"
    And I generate value with date format "YYYYMMdd" and assign to variable "VAR_SYSDATE"
  ...
  Scenario: Verify trade recap SSB file
    ...
    Given I assign "TradeRecapExpected_01_ssb_b1_template.csv" to variable "EXPECTED_FILE_TEMPLATE"
    And I assign "TradeRecapExpected_01_ssb_b1.csv" to variable "EXPECTED_FILE"
...
#Scenarios to test filter query checks
  ...
  Scenario: Publish trade recap file for SSB
    ...
    Given I assign "traderecap_ssb_out_file_b1_02" to variable "PUBLISHING_FILE_NAME"
    And I assign "/dmp/out/ssb" to variable "publishDirectory"
    And I generate value with date format "YYYYMMdd" and assign to variable "VAR_SYSDATE"
  ...
  Scenario: Verify trade recap SSB file
    ...
    Given I assign "TradeRecapExpected_02_ssb_b1_template.csv" to variable "EXPECTED_FILE_TEMPLATE"
    And I assign "TradeRecapExpected_02_ssb_b1.csv" to variable "EXPECTED_FILE"
    And I create input file "${EXPECTED_FILE}" using template "${EXPECTED_FILE_TEMPLATE}" with below codes from location "${testdata.path}/outfiles"
      |  |  |

    Then I expect reconciliation between generated CSV file "${testdata.path}/outfiles/runtime/${PUBLISHING_FILE_NAME}_${VAR_SYSDATE}_1.csv" and reference CSV file "${testdata.path}/outfiles/testdata/TradeRecapExpected_02_ssb_b1.csv" should be successful and exceptions to be written to "${testdata.path}/outfiles/runtime/${PUBLISHING_FILE_NAME}_${VAR_SYSDATE}_001traderecap_exceptions.csv" file