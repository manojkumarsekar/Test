...
@dmp_securities_linking @sec_link_018 @tom_4434
Feature: SCN18:Security Linking Criteria: Data Management Platform (Golden Source)

  Security update on existing security in DMP through any feed file load from BRS/RDM/BNP

  Background:
    Given I assign "tests/test-data/dmp-interfaces/security-linking-dmp/SCN_SECLINK_018" to variable "testdata.path"
    And I assign "SCN_SECLINK__RDM_018.csv" to variable "INPUT_FILENAME1"
    ...
    And I assign "SCN_SECLINK__BRS_018.xml" to variable "INPUT_FILENAME2"
  ...
  Scenario: TC_1: Verify security linking during RDM feed load for ISIN with CURRENCY pair NOT-MATCHING with available data in DMP + SEDOL unavailable CUSIP available and CUSIP + CCY matched
  Update the listings
    ...
    Then I expect value of column "BRS_UPDATE_ISID_COUNT" in the below SQL query equals to "10":
      """
    ...
    Then I expect value of column "BRS_UPDATE_ISID_CUSIP_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS BRS_UPDATE_ISID_CUSIP_COUNT FROM FT_T_ISID
      WHERE INSTR_ID IN (SELECT INSTR_ID FROM FT_T_ISID WHERE ISS_ID = '${ISS_ID}' AND END_TMS IS NULL)
      AND TRUNC(LAST_CHG_TMS) = TRUNC(SYSDATE)
    ...
    Then I expect 0 exceptions are captured with the following criteria
      | PARM_VAL_TXT            | CUSIP – ${BRS_CUSIP}, CURRENCY – ${BRS_CURRENCY} not found in DB. Hence creating listing |
      | NOTFCN_ID               | 60011                                                                                    |
      | SOURCE_ID               | %_GC%                                                                                    |
