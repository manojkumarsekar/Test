...
@dmp_securities_linking @sec_link_027 @tom_4434
Feature: SCN27:Security Linking Criteria: Data Management Platform (Golden Source)

  Security update on existing security in DMP through any feed file load from BRS/RDM/BNP

  Background:
    Given I assign "tests/test-data/dmp-interfaces/security-linking-dmp/SCN_SECLINK_027" to variable "testdata.path"

  Scenario: TC_1:Prerequisite Scenario

    Given I assign "SCN_SECLINK__BNP_027.out" to variable "INPUT_FILENAME1"
    And I assign "SCN_SECLINK__BRS_027.xml" to variable "INPUT_FILENAME2"
    ...
    And I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${BNP_CUSIP}'"
    And I set end_tms to SYSDATE in database "dmp.db.VD" where iss_id in "'${BNP_CUSIP}'"
  ...
  Security update should happen correctly on the right security record in DMP
    ...
    Then I expect value of column "BRS_UPDATE_ISID_COUNT" in the below SQL query equals to "9":
      """
    ...
    Then I expect value of column "BRS_UPDATE_ISID_BCUSIP_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS BRS_UPDATE_ISID_BCUSIP_COUNT FROM FT_T_ISID
      WHERE INSTR_ID IN (SELECT INSTR_ID FROM FT_T_ISID WHERE ISS_ID = '${ISS_ID}' AND END_TMS IS NULL)
      AND TRUNC(LAST_CHG_TMS) = TRUNC(SYSDATE)
  ...
  Scenario: Clear the data after tests
    Then I set end_tms to SYSDATE in database "dmp.db.VD" where iss_id in "'${ISS_ID}','${BNP_CUSIP}','${BNP_BCUSIP}'"
    Then I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${ISS_ID}','${BNP_CUSIP}','${BNP_BCUSIP}'"