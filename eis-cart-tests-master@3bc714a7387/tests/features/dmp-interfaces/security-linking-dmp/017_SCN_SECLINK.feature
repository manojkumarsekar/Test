...
@dmp_securities_linking @sec_link_017 @tom_4434 @ignore
Feature: SCN17:Security Linking Criteria: Data Management Platform (Golden Source)

  Security update on existing security in DMP through any feed file load from BRS/RDM/BNP

  Background:
    Given I assign "tests/test-data/dmp-interfaces/security-linking-dmp/SCN_SECLINK_017" to variable "testdata.path"
    And I assign "SCN_SECLINK__RDM_017.csv" to variable "INPUT_FILENAME1"
    ...
    And I assign "SCN_SECLINK__BRS_017.xml" to variable "INPUT_FILENAME2"

    And I extract value from the xml file "${testdata.path}/data-feeds/${INPUT_FILENAME2}" with tagName "CUSIP" to variable "CUSIP"
    ...
    And I set end_tms to SYSDATE in database "dmp.db.VD" where iss_id in "'${CUSIP}'"
    And I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${CUSIP}'"

    And I set end_tms to SYSDATE in database "dmp.db.VD" where iss_id in "'${BRS_SEDOL}'"
    And I set end_tms to SYSDATE in database "dmp.db.GC" where iss_id in "'${BRS_SEDOL}'"

  Scenario: TC_1:Verify security linking during RDM feed load for ISIN with CURRENCY pair NOT MATCHING with available data in DMP and SEDOL available
  Raise warning and create listing.
    ...
    Then I expect value of column "BRS_ISID_MKT_COUNT" in the below SQL query equals to "6":
      """
    ...
    Then I expect value of column "RDM_UPDATE_ISID_COUNT" in the below SQL query equals to "12":
      """
    ...
    Then I expect value of column "RDM_UPDATE_ISID_COUNT" in the below SQL query equals to "3":
      """
      SELECT COUNT(*) AS RDM_UPDATE_ISID_COUNT FROM FT_T_ISID
      WHERE INSTR_ID IN (SELECT INSTR_ID FROM FT_T_ISID WHERE ISS_ID = '${ISS_ID}' AND END_TMS IS NULL)
      AND TRUNC(LAST_CHG_TMS) = TRUNC(SYSDATE)
    ...
    Then I expect value of column "RDM_UPDATE_MKIS_COUNT" in the below SQL query equals to "2":
      """
      SELECT COUNT(*) AS RDM_UPDATE_MKIS_COUNT
      FROM FT_T_MKIS
      WHERE INSTR_ID = (SELECT INSTR_ID FROM FT_T_ISID WHERE ISS_ID = '${ISS_ID}' AND END_TMS IS NULL)
      AND TRUNC(LAST_CHG_TMS) = TRUNC(SYSDATE)
      """

    Then I expect value of column "RDM_UPDATE_ISID_MKT1_COUNT" in the below SQL query equals to "3":
      """
      SELECT COUNT(*) AS RDM_UPDATE_ISID_MKT1_COUNT FROM FT_T_ISID A
      JOIN FT_T_MKIS B
      ON A.MKT_OID=B.MKT_OID
      JOIN FT_T_ISID C
      ON A.INSTR_ID=C.INSTR_ID AND B.INSTR_ID=C.INSTR_ID AND C.ISS_ID = '${ISS_ID}' AND C.END_TMS IS NULL
    ...
    Then I expect value of column "RDM_UPDATE_ISID_MKT2_COUNT" in the below SQL query equals to "6":
      """
      SELECT COUNT(*) AS RDM_UPDATE_ISID_MKT2_COUNT FROM FT_T_ISID A
      JOIN FT_T_MKIS B
      ON A.MKT_OID=B.MKT_OID
      JOIN FT_T_ISID C
      ON A.INSTR_ID=C.INSTR_ID AND B.INSTR_ID=C.INSTR_ID AND C.ISS_ID = '${ISS_ID}' AND C.END_TMS IS NULL
    ...
    Then I expect value of column "RDM_UPDATE_ISID_SEDOL_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS RDM_UPDATE_ISID_SEDOL_COUNT FROM FT_T_ISID
      WHERE INSTR_ID IN (SELECT INSTR_ID FROM FT_T_ISID WHERE ISS_ID = '${ISS_ID}' AND END_TMS IS NULL)
      AND TRUNC(LAST_CHG_TMS) = TRUNC(SYSDATE)
      AND ID_CTXT_TYP = 'SEDOL'
      """

    Then I expect value of column "RDM_UPDATE_MKID_COUNT" in the below SQL query equals to "4":
      """
      SELECT COUNT(*) AS RDM_UPDATE_MKID_COUNT FROM FT_T_MKID A
      JOIN FT_T_MKIS B
      ON A.MKT_OID = B.MKT_OID
      JOIN FT_T_ISID C
      ON B.INSTR_ID = C.INSTR_ID
    ...
    Then I expect value of column "RDM_UPDATE_MKID_COUNT" in the below SQL query equals to "1":
      """
      SELECT COUNT(*) AS RDM_UPDATE_MKID_COUNT FROM FT_T_MKID A
      JOIN FT_T_MKIS B
      ON A.MKT_OID = B.MKT_OID
