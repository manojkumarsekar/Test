...
--  ==================================================================
    BEGIN TRY
        ...
        SET @SQL = 'SELECT DISTINCT name FROM sys.dm_exec_describe_first_result_set (''SELECT ' + @MatchKeySourceList + ' FROM ' + @SourceTable + ''', NULL, 0)' ;

        INSERT INTO #MatchKeySourceList(ColumnName)
        EXEC(@SQL);
        ...
        IF @ColumnList <> ''
        BEGIN
            SET @ErrorMessage = 'Column(s) "' + @ColumnList + '" is/are marked as Match Key but is part of the Excluded Columns in either Source or Target. Match Key and Excluded Columns are mutually exclusive';
            RAISERROR(@ErrorMessage, 16, 1);
        END;
        ------------------------------------------------------------------------
        ...
        END;
        ------------------------------------------------------------------------
        -- Are there any Source/ Target columns remaining after all exclusions?
        IF NOT EXISTS
        (
            SELECT '1'
            FROM TestAutomation.dbo.ColumnCategory with (NOLOCK)
        ...
        IF @ColumnList <> ''
        BEGIN
            SET @ErrorMessage = 'All columns in Source/ Target tables appear to be excluded.';
            RAISERROR(@ErrorMessage, 16, 1);
        END;
        ------------------------------------------------------------------------
        ...
        ORDER BY ISNULL(KeyOrder, 9999) ASC, ColumnOrder ASC;
...
IF OBJECT_ID('TestAutomation.dbo.LoadQuery', 'P') IS NOT NULL DROP PROCEDURE dbo.LoadQuery;
GO
SET QUOTED_IDENTIFIER ON;
GO
CREATE PROCEDURE dbo.LoadQuery
(
    @Query VARCHAR(MAX),
...
BEGIN
    ...
    -- =========    ===========     =========
    -- SankaranS    4-April-2017    Initial Version
    ...
    BEGIN TRY
        ...
        IF ISNULL(@Query, '') = ''
        BEGIN
            SET @ErrorMessage = 'Error: Invalid value for parameter @Query'
            RAISERROR(@ErrorMessage, 16, 1)
        END
        --------------------------------------------------------------------
        ...
        IF @LinkedServer <> ''
        BEGIN
            IF NOT EXISTS(SELECT name FROM sys.servers WHERE name = @LinkedServer)
            BEGIN
                SET @ErrorMessage = 'Error: LinkedServer not defined in the Host Server'
                RAISERROR(@ErrorMessage, 16, 1)
            END

            SET @SQL = 'SELECT * INTO ' + @TableName  + ' FROM OPENQUERY(' + @LinkedServer + ', ''' + @Query + ''')'
            EXEC(@SQL)
        END
        ELSE
        BEGIN
            SET @SQL = 'SELECT * INTO ' + @TableName  + ' FROM OPENROWSET(''SQLNCLI'', ''Server=' + @Server + ';Database=' + @Database + ';UID=ReconUser;PWD=ReconUser1234;'', ''' + @Query + ''') AS X;'
            EXEC(@SQL)
        END
    END TRY
